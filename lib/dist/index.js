...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderGroupChart = function (chart, data) {
        d3.select("#" + chart.id + " .card-subtitle")
            .html("The reflections of the group code " + data[d3.minIndex(data.map(function (c) { return c.stats.find(function (d) { return d.stat == "q1"; }).value; }))].group + " tends to be distressed, while\n                the reflections of the group code " + data[d3.maxIndex(data.map(function (c) { return c.stats.find(function (d) { return d.stat == "q3"; }).value; }))].group + " tends to be soaring");
    ...
    AdminControlCharts.prototype.renderHistogram = function (chart, data) {
        ...
        d3.select("#" + chart.id + " .card-subtitle")
            .html("The group code " + binData[d3.maxIndex(binData.map(function (d) { return d3.max(d.filter(function (d) { return d.bin.x0 == 0; }).map(function (d) { return d.percentage; })); }))][0].group + " has the highest percentage in the distressed bin, while\n                the group code " + binData[d3.maxIndex(binData.map(function (d) { return d3.max(d.filter(function (d) { return d.bin.x1 == 100; }).map(function (d) { return d.percentage; })); }))][0].group + " has the highest percentage in the soaring bin");
        //Process histogram
    ...
    ;
    AdminControlCharts.prototype.renderUserStatistics = function (data, pseudonym) {
        ...
        d3.select("#user-statistics").append("div")
            ...
            .enter()
            .append("div")
            .attr("class", function (d, i) { return "tab-pane fade " + (i == 0 ? "show active" : ""); })
            .append("span")
            .attr("class", "badge badge-pill badge-light")
            .html(function (d) { return d.pseudonym + " - " + d.point + " (" + ((d.point - groupMean) < 0 ? "" : "+") + (d.point - groupMean) + Math.abs(d.point - groupMean) + ")"; });
    };
    ;
    AdminControlCharts.prototype.renderUserReflections = function (card, data, pseudonym) {
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    ;
    AdminExperimentalCharts.prototype.renderUserReflections = function (card, data, pseudonym) {
        _super.prototype.renderUserReflections.call(this, card, data, pseudonym);
        var _this = this;
        card.selectAll("a")
...
function buildControlAdminAnalyticsCharts(entriesRaw) {
    ...
    function drawCharts(allEntries) {
        ...
        timelineCard.select(".card-subtitle")
            .html("The oldest reflection was on " + d3.min(data.map(function (d) { return d.getStat("oldRef").value; })).toDateString() + " in the group code " + data[d3.minIndex(data.map(function (d) { return d.getStat("oldRef").value; }))].group + ", while\n                the newest reflection was on " + d3.max(data.map(function (d) { return d.getStat("newRef").value; })).toDateString() + " in the group code " + data[d3.maxIndex(data.map(function (d) { return d.getStat("newRef").value; }))].group);
        ...
        var userStatistics = adminControlCharts.htmlContainers.appendCard(adminControlCharts.htmlContainers.userStatistics, "Users average reflection point compared to its group", "user-statistics", true);
        ...
        var usersCards = adminControlCharts.htmlContainers.userReflectionText.selectAll("div")
            .data(data)
            .enter()
            .append("div")
            .attr("class", "card");
