...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderTotals = function (data) {
        d3.select("#users-total .card-title span")
            .html(d3.sum(data.map(function (d) { return d.getStat("usersTotal").value; })).toString());
        d3.select("#ref-total .card-title span")
            .html(d3.sum(data.map(function (d) { return d.getStat("refTotal").value; })).toString());
        d3.select("#ru-rate .card-title span")
            .html(Math.round(d3.mean(data.map(function (d) { return d.getStat("ruRate").value; }))).toString());
    };
    ;
    AdminControlCharts.prototype.renderBarChart = function (chart, data) {
        ...
        chart.elements.content = chart.elements.contentContainer.selectAll("#" + chart.id + "-data")
            ...
            .attr("x", function (d) { return chart.x.scale(d.group); })
            .attr("width", chart.x.scale.bandwidth())
            .attr("height", function (d) { return chart.y.scale(d.getStat("usersTotal").value); }); }); }, function (exit) { return exit.style("fill", "#cccccc")
            .style("stroke", "#b3b3b3")
    ...
    AdminControlCharts.prototype.renderUserStatistics = function (card, data, thresholds, timelineData) {
        ...
        function drawUserChart(id, pseudonym, thresholds) {
            ...
            chart.elements.contentContainer.selectAll("#group")
                .data(bin(usersData.value.map(function (d) { return d.point; })).map(function (c) { return new HistogramData(usersData.value, usersData.group, usersData.colour, c, Math.round(c.length / usersData.value.length * 100)); }))
                .join(function (enter) { return enter.append("circle")
                .attr("id", "group")
                .attr("class", "circle-group")
                .attr("r", 5)
                .attr("cx", function (d) { return chart.x.scale(d.percentage); })
                .attr("cy", function (d) { return (d.bin.x0 == 0 ? chart.y.scale("distressed") : d.bin.x1 == 100 ? chart.y.scale("soaring") : chart.y.scale("going ok")) + chart.y.scale.bandwidth() / 2; })
                .attr("fill", function (d) { return d.colour; })
                .attr("stroke", function (d) { return d.colour; }); });
            chart.elements.contentContainer.selectAll("#user")
                .data(bin(userData.map(function (d) { return d.point; })).map(function (c) { return new HistogramData(userData, usersData.group, usersData.colour, c, Math.round(c.length / userData.length * 100)); }))
                .join(function (enter) { return enter.append("circle")
                .attr("id", "user")
    ...
    AdminExperimentalCharts.prototype.renderHistogram = function (chart, data) {
        ...
        function dragEnd() {
            ...
            if (chart.id == "group-histogram-users-chart" && !_this.timeline.elements.contentContainer.selectAll(".clicked").empty()) {
                ...
                d3.select("#reflections #" + usersData_1.pseudonym + " .text-muted")
                    .html("<b>" + binName + "</b>");
            }
        }
        _this.interactions.click.enableClick(chart, onClick);
            ...
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    d3.select("#group-histogram-users-chart .card-title button")
                        ...
                        var showDragHelp = adminExperimentalCharts.help.helpPopover(adminExperimentalCharts.usersHistogram.elements.contentContainer.select(".threshold-line.soaring"), adminExperimentalCharts.usersHistogram.id + "-help-drag", "<u><i>drag</i></u> me to change the thresholds");
                        if (showDragHelp) {
