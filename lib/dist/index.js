...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderGroupChart = function (chart, data) {
        ...
        chart.elements.contentContainer.selectAll("#" + chart.id + "-data-min-max")
            ...
            .attr("y2", function (d) { return chart.y.scale(d.max); }); }); }, function (update) { return update.style("stroke", function (d) { return d.colour; })
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("x1", function (d) { return chart.x.scale(d.group) + (chart.x.scale.bandwidth() / 2); })
            .attr("x2", function (d) { return chart.x.scale(d.group) + (chart.x.scale.bandwidth() / 2); })
            .attr("y1", function (d) { return chart.y.scale(d.min); })
            .attr("y2", function (d) { return chart.y.scale(d.max); }); }); }, function (exit) { return exit.style("stroke", "#b3b3b3")
            .call(function (exit) { return exit.transition()
            .duration(250)
            .attr("y2", function (d) { return chart.y.scale(d.min); })
            .remove(); }); });
        ...
        chart.elements.contentContainer.selectAll("#" + chart.id + "-data-median")
            ...
            .attr("y1", function (d) { return chart.y.scale(d.median); })
            .attr("y2", function (d) { return chart.y.scale(d.median); }); }); }, function (exit) { return exit.style("stroke", "#b3b3b3")
            .call(function (exit) { return exit.transition()
            .duration(250)
        ...
        chart.elements.content = chart.elements.contentContainer.selectAll("#" + chart.id + "-data")
            ...
            .attr("class", "bar")
            .attr("y", function (d) { return chart.y.scale(d.q1); })
            ...
            .attr("y", function (d) { return chart.y.scale(d.q3); }); }); }, function (update) { return update.style("stroke", function (d) { return d.colour; })
            .style("fill", function (d) { return d.colour; })
            .call(function (update) { return update.transition()
            .duration(750)
            ...
            .attr("x", function (d) { return chart.x.scale(d.group); })
            .attr("width", chart.x.scale.bandwidth())
            .attr("height", function (d) { return chart.y.scale(d.q1) - chart.y.scale(d.q3); }); }); }, function (exit) { return exit.style("fill", "#cccccc")
            .style("stroke", "#b3b3b3")
            .call(function (exit) { return exit.transition()
            .duration(250)
            .attr("y", function (d) { return chart.y.scale(d.q1); })
    ...
    AdminExperimentalCharts.prototype.handleGroupsSort = function (boxPlot) {
        ...
        d3.select("#groups-chart #sort-by").on("click", function (e) {
            ...
            function getClickData() {
                if (!boxPlot.elements.contentContainer.select(".clicked").empty) {
                    return boxPlot.elements.contentContainer.select(".clicked").datum();
                }
                return;
            }
            _this.renderGroupChart(boxPlot, data);
            if (boxPlot.click) {
                ...
                var histogramData = _this.getGroupCompareData();
                _this.interactions.axisSeries(_this.histogram, histogramData);
                var histogramClickData = _this.histogram.elements.contentContainer.select(".clicked").datum();
                _this.renderHistogram(_this.histogram, histogramData);
                var usersData = histogramData.map(function (d) { return d.getUsersData(); });
                _this.interactions.axisSeries(_this.usersHistogram, usersData);
                var usersHistogramClickData = _this.histogram.elements.contentContainer.select(".clicked").datum();
                _this.renderHistogram(_this.usersHistogram, usersData);
...
var Click = /** @class */ (function () {
    ...
    Click.prototype.appendGroupsText = function (chart, data, clickData) {
        ...
        chart.elements.contentContainer.selectAll(".click-container")
            ...
            .data(function (c) { return [{ title: "q3", clickData: clickData.q3, data: c.q3, clickGroup: clickData.group, dataGroup: c.group },
            ...
            .enter()
            .append("text")
            .attr("class", function (c) { return _this_1.comparativeText(c.clickData, c.data, c.clickGroup, c.dataGroup)[0]; })
            .attr("y", function (c) { return chart.y.scale(c.data) - 5; })
            .text(function (c) { return c.title + ": " + _this_1.comparativeText(c.clickData, c.data, c.clickGroup, c.dataGroup)[1]; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (c) { return "translate(" + (chart.x.scale(c.group) + chart.x.scale.bandwidth() / 2) + ", 0)"; }); })
            ...
            .attr("class", function (c) { return _this_1.comparativeText(c.clickData, c.data, c.clickGroup, c.dataGroup)[0]; })
            .attr("y", function (c) { return chart.y.scale(c.data) - 5; })
            .text(function (c) { return c.title + ": " + _this_1.comparativeText(c.clickData, c.data, c.clickGroup, c.dataGroup)[1]; }); }); }, function (exit) { return exit; }); }); }, function (exit) { return exit.remove(); });
    };
    ;
    Click.prototype.appendThresholdPercentages = function (chart, data, clickData) {
        ...
        chart.elements.contentContainer.selectAll(".click-container")
            ...
            .enter()
            .append("text")
            .attr("class", function (c) { return _this_1.comparativeText(clickData.bin.length, c.bin.length, clickData.group, c.group)[0]; })
            ...
            .text(function (c) { return _this_1.comparativeText(clickData.bin.length, c.bin.length, clickData.group, c.group)[1] + " (" + Math.abs(clickData.percentage - c.percentage) + "%)"; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (c) { return "translate(" + (chart.x.scale(c.group) + chart.x.scale.bandwidth() / 2) + ", 0)"; }); })
            .call(function (update) { return update.selectAll("text")
            .data(function (d) { return chart.bin(d.value.map(function (d) { return d.point; })).map(function (c) { return new HistogramData(d.group, d.colour, c, Math.round(c.length / d.value.length * 100)); }); })
            ...
            .attr("class", function (c) { return _this_1.comparativeText(clickData.bin.length, c.bin.length, clickData.group, c.group)[0]; })
            .attr("y", function (c) { return c.bin.x0 == 0 ? chart.y.scale(0 + tDistressed / 2) : c.bin.x1 == 100 ? chart.y.scale(tSoaring + (100 - tSoaring) / 2) : chart.y.scale(50); })
            .text(function (c) { return _this_1.comparativeText(clickData.bin.length, c.bin.length, clickData.group, c.group)[1] + " (" + Math.abs(clickData.percentage - c.percentage) + "%)"; }); }); }, function (exit) { return exit; }); }); }, function (exit) { return exit.remove(); });
    };
    ;
