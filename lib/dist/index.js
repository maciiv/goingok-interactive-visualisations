...
var ViolinChartElements = /** @class */ (function (_super) {
    ...
    ViolinChartElements.prototype.appendThresholdIndicators = function (chart, thresholds) {
        this.contentContainer.selectAll(".threshold-indicator-container")
            ...
            .attr("transform", function (d) { return "translate(" + (chart.width - chart.padding.yAxis - chart.padding.right + 5) + ", " + (chart.y.scale(d) + 25) + ")"; })
...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderGroupChart = function (chart, data) {
        ...
        chart.elements.contentContainer.selectAll("#" + chart.id + "-data-median")
            ...
            .remove(); }); });
        //Boxes processing
        chart.elements.contentContainer.selectAll("#" + chart.id + "-data")
            .data(data)
    ...
    AdminControlCharts.prototype.renderViolin = function (chart, data) {
        ...
        chart.elements.contentContainer.selectAll("." + chart.id + "-violin-container")
            .data(data)
            ...
            .call(function (enter) { return enter.selectAll(".violin-rect")
            .data(function (d) { return chart.bin(d.value.map(function (d) { return d.point; })).map(function (c) { return { binData: c, colour: d.colour }; }); })
            .enter()
            .append("rect")
            .attr("id", chart.id + "-data")
            .attr("class", "violin-rect")
            .attr("x", function (c) { return chart.bandwidth(-c.binData.length); })
            .attr("y", function (c) { return chart.y.scale(c.binData.x0); })
            .attr("height", 0)
            .attr("width", function (c) { return chart.bandwidth(c.binData.length) - chart.bandwidth(-c.binData.length); })
            .style("stroke", function (c) { return c.colour; })
            .style("fill", function (c) { return c.colour; })
            .transition()
            .duration(750)
            .attr("y", function (c) { return chart.y.scale(c.binData.x1); })
            .attr("height", function (c) { return chart.y.scale(c.binData.x0) - chart.y.scale(c.binData.x1); }); }); }, function (update) { return update
            .call(function (update) { return _this_1.interactions.violin(chart, update); })
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (d) { return "translate(" + chart.x.scale(d.group) + ", 0)"; }); }); }, function (exit) { return exit
            .call(function (exit) { return exit.selectAll(".violin-rect")
            .style("fill", "#cccccc")
            .style("stroke", "#b3b3b3")
            .transition()
            .duration(250)
...
var AdminControlTransitions = /** @class */ (function () {
    ...
    ;
    AdminControlTransitions.prototype.violin = function (chart, update) {
        update.selectAll(".violin-rect")
            ...
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("x", function (d) { return chart.bandwidth(-d.binData.length); })
            .attr("y", function (d) { return chart.y.scale(d.binData.x1); })
            .attr("height", function (d) { return chart.y.scale(d.binData.x0) - chart.y.scale(d.binData.x1); })
            .attr("width", function (d) { return chart.bandwidth(d.binData.length) - chart.bandwidth(-d.binData.length); }); }); }, function (exit) { return exit; });
    };
    ;
    ...
    AdminExperimentalCharts.prototype.renderViolin = function (chart, data) {
        ...
        function draggingSoaring(e, d) {
            ...
            d3.select(this)
                ...
                .tickFormat(function (d) { return d == tDistressed ? "Distressed" : d == line.datum() ? "Soaring" : ""; }); })
                .call(function (line) { return chart.elements.contentContainer.selectAll(".threshold-axis")
                .call(chart.thresholdAxis); })
                .call(function (line) { return chart.elements.contentContainer.select(".threshold-indicator-container.soaring")
