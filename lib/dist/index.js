...
var AuthorControlCharts = /** @class */ (function () {
    ...
    AuthorControlCharts.prototype.renderNetwork = function (chart, entries, data) {
        ...
        function ticked(chart) {
            chart.elements.contentContainer.selectAll(".network-link").transition()
                .duration(750)
            ...
            chart.elements.contentContainer.selectAll(".network-node").transition()
                .duration(750)
                .attr("x", function (d) { return d.x - ((chart.elements.contentContainer.selectAll("#text-" + d.index).node().getBoundingClientRect().width + 10) / 2); })
                .attr("y", function (d) { return d.y - ((chart.elements.contentContainer.selectAll("#text-" + d.index).node().getBoundingClientRect().height + 5) / 2); })
                .attr("width", function (d) { return chart.elements.contentContainer.selectAll("#text-" + d.index).node().getBoundingClientRect().width + 10; })
                .attr("height", function (d) { return chart.elements.contentContainer.selectAll("#text-" + d.index).node().getBoundingClientRect().height + 5; });
        }
        ...
        chart.elements.contentContainer.selectAll(".network-link")
            ...
            .attr("class", "network-link"); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        chart.elements.contentContainer.selectAll(".network-node")
            .data(data.tags)
            .join(function (enter) { return enter.append("rect")
            .attr("class", function (d) { return "network-node " + d.tag.toLowerCase(); }); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        chart.elements.contentContainer.selectAll(".network-text")
            .data(data.tags)
            .join(function (enter) { return enter.append("text")
            .attr("class", "network-text")
            .text(function (d) { return d.phrase; }); }, function (update) { return update; }, function (exit) { return exit.remove(); });
