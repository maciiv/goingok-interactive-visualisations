...
var AnalyticsChartsDataStats = /** @class */ (function (_super) {
    ...
    function AnalyticsChartsDataStats(entries) {
        ...
        _this_1.stats.push(new DataStats("variance", "Variance", _this_1.roundDecimal(d3.variance(entries.value.map(function (r) { return r.point; })))));
        _this_1.stats.push(new DataStats("deviation", "Std Deviation", _this_1.roundDecimal(d3.deviation(entries.value.map(function (r) { return r.point; })))));
        _this_1.stats.push(new DataStats("oldRef", "Oldest reflection", d3.min(entries.value.map(function (r) { return new Date(r.timestamp); })).toDateString()));
        _this_1.stats.push(new DataStats("newRef", "Newest reflection", d3.max(entries.value.map(function (r) { return new Date(r.timestamp); })).toDateString()));
        _this_1.stats.push(new DataStats("totalRef", "Total reflections", d3.max(entries.value.map(function (r) { return new Date(r.timestamp); })).toDateString()));
        _this_1.stats.push(new DataStats("avgRef", "Reflections per user", _this_1.roundDecimal(d3.mean(uniqueUsers.map(function (r) { return r.value; })))));
        _this_1.stats.push(new DataStats("userHRef", "Max reflections per user", d3.max(uniqueUsers.map(function (r) { return r.value; }))));
        _this_1.stats.push(new DataStats("userLRef", "Min reflections per user", d3.min(uniqueUsers.map(function (r) { return r.value; }))));
...
var ChartElements = /** @class */ (function () {
    ...
    ChartElements.prototype.appendSVG = function (chart) {
        var svg = d3.select("#" + chart.id + " .chart-container")
            .append("svg")
...
var HtmlContainers = /** @class */ (function () {
    ...
    HtmlContainers.prototype.appendCard = function (div, header, id, help) {
        ...
        return div.append("div")
            ...
            .call(function (div) { return div.append("div").attr("class", "card-body chart-container"); });
    };
    ;
...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderGroupChart = function (chart, data) {
        ...
        function onMouseover(e, d) {
            ...
            var tooltipBox = _this.interactions.tooltip.appendTooltipText(chart, d.group, d.stats.filter(function (c, i) { return i < 6; }).map(function (c) { return new TooltipValues(c.stat, c.value); }));
    ...
    AdminControlCharts.prototype.renderGroupStats = function (div, data) {
        ...
        return div.select(".card-body")
            ...
            .append("p")
            .attr("class", "my-0")
            .html(function (d) { return "<b>" + d.displayName + ": </b>" + d.value; });
    };
    ;
    AdminControlCharts.prototype.renderHistogram = function (chart, data) {
        ...
        chart.elements.contentContainer.selectAll("." + chart.id + "-histogram-container")
            ...
            .attr("x", function (c) { return chart.bandwidth(-c.bin.length); })
            .attr("y", function (c) { return chart.y.scale(c.bin.x0); })
            .attr("height", 0)
            .attr("width", function (c) { return chart.bandwidth(c.bin.length) - chart.bandwidth(-c.bin.length); })
    ...
    AdminControlCharts.prototype.renderTimelineDensity = function (chart, data) {
        ...
        function drawContours() {
            ...
            chart.elements.content = chart.elements.contentContainer.selectAll("#" + chart.id + "-timeline-contours")
                ...
                .attr("fill", function (d) { return d3.interpolateRgb("#ffffff", data.colour)(d.value * 20); }); }, function (update) { return update.attr("d", d3.geoPath())
                .attr("stroke", function (d) { return d3.interpolateRgb("#ffffff", data.colour)(d.value * 25); })
                .attr("fill", function (d) { return d3.interpolateRgb("#ffffff", data.colour)(d.value * 20); }); }, function (exit) { return exit.remove(); });
        }
        //Enable zoom
    ...
    AdminControlCharts.prototype.renderTimelineScatter = function (chart, zoomChart, data) {
        ...
        chart.elements.content = chart.elements.contentContainer.selectAll("#" + chart.id + "-timeline-circles")
            ...
            .style("fill", data.colour); }, function (update) { return update.style("stroke", data.colour)
        ...
        chart.elements.zoomSVG.selectAll("#" + chart.id + "-zoom-bar-content")
            ...
            .duration(750)
            .attr("cx", function (d) { return zoomChart.x.scale(d.timestamp); })
            .attr("cy", function (d) { return zoomChart.y.scale(d.point); }); }); }, function (exit) { return exit.remove(); });
        chart.elements.zoomFocus.selectAll("#" + chart.id + "-zoom-content")
            ...
            .attr("r", 2)
            .attr("cx", function (d) { return zoomChart.x.scale(d.timestamp); })
            .attr("cy", function (d) { return zoomChart.y.scale(d.point); }); }, function (update) { return update.attr("cx", function (d) { return zoomChart.x.scale(d.timestamp); })
            .attr("cy", function (d) { return zoomChart.y.scale(d.point); }); }, function (exit) { return exit.remove(); });
        //Enable zoom
...
var AdminControlTransitions = /** @class */ (function () {
    ...
    AdminControlTransitions.prototype.histogram = function (chart, update) {
        update.selectAll(".histogram-rect")
            ...
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("x", function (d) { return chart.bandwidth(-d.bin.length); })
            ...
            .attr("width", function (d) { return chart.bandwidth(d.bin.length) - chart.bandwidth(-d.bin.length); }); }); }, function (exit) { return exit; });
    };
    ;
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroups = function (boxPlot) {
        ...
        d3.selectAll("#groups input[type=checkbox]").on("change", function (e) {
            ...
            if (target.checked) {
                ...
                if (boxPlot.click) {
                    var clickData = boxPlot.elements.contentContainer.select(".clicked").datum();
                ...
                if (boxPlot.click) {
                    if (target.value == d3.select("#groups-statistics .card").attr("id")) {
                        ...
                        var clickData = boxPlot.elements.contentContainer.select(".clicked").datum();
                        _this.interactions.click.appendGroupsText(boxPlot, data, clickData);
                        var histogramData = _this.getGroupCompareData();
                        _this.renderGroupCompare();
                        _this.histogram.x.scale.domain(histogramData.map(function (r) { return r.group; }));
                        _this.usersHistogram.x.scale.domain(histogramData.map(function (r) { return r.group; }));
                        _this.handleGroupCompare();
    ...
    AdminExperimentalCharts.prototype.handleGroupsColours = function (boxPlot) {
        ...
        d3.selectAll("#groups input[type=color]").on("change", function (e) {
            ...
            if (boxPlot.click) {
                ...
                var histogramData = _this.getGroupCompareData();
                if (histogramData.map(function (d) { return d.group; }).includes(groupId)) {
                    _this.renderHistogram(_this.histogram, histogramData);
                    var usersData = histogramData.map(function (d) { return d.getUsersData(); });
    ...
    AdminExperimentalCharts.prototype.handleGroupsSort = function (boxPlot) {
        ...
        d3.select("#groups-chart #sort-by").on("click", function (e) {
            ...
            function getClickData(contentContainer) {
                if (!contentContainer.select(".clicked").empty()) {
                    return contentContainer.select(".clicked").datum();
                }
                return;
            }
            var groupClickData = getClickData(boxPlot.elements.contentContainer);
            _this.renderGroupChart(boxPlot, data);
            if (boxPlot.click) {
                _this.interactions.click.appendGroupsText(boxPlot, data, groupClickData);
                var histogramData = _this.getGroupCompareData();
                _this.interactions.axisSeries(_this.histogram, histogramData);
                var histogramClickData = getClickData(_this.histogram.elements.contentContainer);
                _this.renderHistogram(_this.histogram, histogramData);
                var usersData = histogramData.map(function (d) { return d.getUsersData(); });
                _this.interactions.axisSeries(_this.usersHistogram, usersData);
                var usersHistogramClickData = getClickData(_this.usersHistogram.elements.contentContainer);
                _this.renderHistogram(_this.usersHistogram, usersData);
    ...
    AdminExperimentalCharts.prototype.renderGroupChart = function (chart, data) {
        ...
        function onClick(e, d) {
            ...
            _this.histogram = new HistogramChartSeries("group-histogram-chart", histogramData.map(function (d) { return d.group; }));
            ...
            _this.htmlContainers.histogram.select(".card-header button")
                .on("click", function (e) {
                var showHelp = _this.htmlContainers.helpPopover(d3.select(this), _this.histogram.id + "-help");
                ...
                var showDragHelp = _this.htmlContainers.helpPopover(_this.histogram.elements.contentContainer.select(".threshold-line.soaring"), _this.histogram.id + "-help-drag", "drag me!");
                if (showDragHelp) {
            ...
            var usersData = histogramData.map(function (d) { return d.getUsersData(); });
            _this.usersHistogram = new HistogramChartSeries("group-histogram-users-chart", histogramData.map(function (d) { return d.group; }));
            ...
            _this.htmlContainers.userHistogram.select(".card-header button")
                .on("click", function (e) {
                var showHelp = _this.htmlContainers.helpPopover(d3.select(this), _this.usersHistogram.id + "-help");
                ...
                var showDragHelp = _this.htmlContainers.helpPopover(_this.usersHistogram.elements.contentContainer.select(".threshold-line.soaring"), _this.usersHistogram.id + "-help-drag", "drag me!");
                if (showDragHelp) {
    ...
    AdminExperimentalCharts.prototype.handleGroupCompare = function () {
        ...
        d3.selectAll("#group-compare input").on("change", function (e, d) {
            ...
            _this.histogram.x.scale.domain(groupData.map(function (r) { return r.group; }));
            _this.usersHistogram.x.scale.domain(groupData.map(function (r) { return r.group; }));
            ...
            if (_this.histogram.click) {
                var clickData = _this.histogram.elements.contentContainer.select(".clicked").datum();
...
var Click = /** @class */ (function () {
    ...
    Click.prototype.appendGroupsText = function (chart, data, clickData) {
        ...
        chart.elements.contentContainer.selectAll(".click-container")
            ...
            .text(function (c) { return "(" + _this_1.comparativeText(c)[1] + ")"; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (c) { return "translate(" + (chart.x.scale(c.group) + chart.x.scale.bandwidth() / 2) + ", 0)"; }); })
            .call(function (update) { return update.selectAll("text")
            .data(function (c) { return c.stats.filter(function (d) { return d.stat == "q3" || d.stat == "median" || d.stat == "q1"; }).map(function (d) { return new ClickTextData(clickData.stats.find(function (a) { return a.stat == d.stat; }), d, clickData.group, c.group); }); })
            .join(function (enter) { return enter; }, function (update) { return update.call(function (text) { return text.selectAll("tspan")
            .attr("class", function (c) { return _this_1.comparativeText(c)[0]; })
            .text(function (c) { return "(" + _this_1.comparativeText(c)[1] + ")"; }); })
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("class", "click-text black")
            .attr("y", function (c) { return chart.y.scale(c.data.stat.value) - 5; })
            .text(function (c) { return c.data.stat.displayName + ": " + c.data.stat.value + " "; }); }); }, function (exit) { return exit; }); }); }, function (exit) { return exit.remove(); });
    };
    ;
    Click.prototype.appendThresholdPercentages = function (chart, data, clickData) {
        ...
        chart.elements.contentContainer.selectAll(".click-container")
            ...
            .enter()
            .append("text")
            .attr("class", function (c) { return _this_1.comparativeText(new ClickTextData(clickData.bin.length, c.bin.length, clickData.group, c.group))[0]; })
            ...
            .text(function (c) { return _this_1.comparativeText(new ClickTextData(clickData.bin.length, c.bin.length, clickData.group, c.group))[1] + " (" + Math.abs(clickData.percentage - c.percentage) + "%)"; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (c) { return "translate(" + (chart.x.scale(c.group) + chart.x.scale.bandwidth() / 2) + ", 0)"; }); })
            .call(function (update) { return update.selectAll("text")
            .data(function (d) { return chart.bin(d.value.map(function (d) { return d.point; })).map(function (c) { return new HistogramData(d.group, d.colour, c, Math.round(c.length / d.value.length * 100)); }); })
            .join(function (enter) { return enter; }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("class", function (c) { return _this_1.comparativeText(new ClickTextData(clickData.bin.length, c.bin.length, clickData.group, c.group))[0]; })
            .attr("y", function (c) { return c.bin.x0 == 0 ? chart.y.scale(0 + tDistressed / 2) : c.bin.x1 == 100 ? chart.y.scale(tSoaring + (100 - tSoaring) / 2) : chart.y.scale(50); })
            .text(function (c) { return _this_1.comparativeText(new ClickTextData(clickData.bin.length, c.bin.length, clickData.group, c.group))[1] + " (" + Math.abs(clickData.percentage - c.percentage) + "%)"; }); }); }, function (exit) { return exit; }); }); }, function (exit) { return exit.remove(); });
    };
    ;
...
function buildControlAdminAnalyticsCharts(entriesRaw) {
    ...
    function drawCharts(allEntries) {
        ...
        adminControlCharts.htmlContainers.statistics.selectAll("div")
            ...
            .attr("data-target", function (d) { return "#stats-" + d.group; })
            .attr("data-toggle", "collapse")
            .html(function (d) { return d.group + " statistics"; }); })
            .call(function (div) { return div.append("div")
            .attr("id", function (d) { return "stats-" + d.group; })
            .attr("class", function (d, i) { return "collapse " + (i == 0 ? "show" : ""); })
            .attr("data-parent", "#groups-statistics")
            .append("div")
            .attr("class", "card-body"); })
            .call(function (div) { return adminControlCharts.renderGroupStats(div, div.datum()); });
        //Draw groups histogram container  
        ...
        adminControlCharts.htmlContainers.histogram.select(".card-header button")
            .on("click", function (e) {
            var showHelp = adminControlCharts.htmlContainers.helpPopover(d3.select(this), histogramChart.id + "-help");
        ...
        adminControlCharts.htmlContainers.userHistogram.select(".card-header button")
            .on("click", function (e) {
            var showHelp = adminControlCharts.htmlContainers.helpPopover(d3.select(this), histogramUsersChart.id + "-help");
        ...
        timelineCard.select(".card-body")
            .append("div")
            .attr("class", "row mt-3")
            .html(function () { return "<div id=\"timeline-plot\" class=\"btn-group btn-group-toggle mr-auto ml-auto\" data-toggle=\"buttons\">\n                            <label class=\"btn btn-light active\">\n                                <input type=\"radio\" name=\"plot\" value=\"density\" checked>Density Plot<br>\n                            </label>\n                            <label class=\"btn btn-light\">\n                                <input type=\"radio\" name=\"plot\" value=\"scatter\">Scatter Plot<br>\n                            </label>\n                        </div>"; });
