...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroupsColours = function (chart) {
        ...
        d3.selectAll("#groups input[type=color]").on("change", function (e) {
            var target = e.target;
            var group = target.id.replace("colour-", "");
            _this.allEntries.find(function (d) { return d.group == group; }).colour = target.value;
            var entries = d3.filter(_this.allEntries, function (d) { return d.selected; });
            var data = entries.map(function (d) { return new AnalyticsChartsDataStats(d); });
            _this.renderGroupChart(chart, data);
            if (chart.click) {
                ...
                if (currentClickGroup == group) {
                    ...
                    if (_this.timeline.elements.contentContainer.selectAll("#" + _this.timeline.id + "-timeline-contours").empty()) {
                        _this.renderTimelineScatter(_this.timeline, _this.timelineZoom, _this.allEntries.find(function (d) { return d.group == group; }));
                    }
                    else {
                        _this.timeline.elements.contentContainer.selectAll("#" + _this.timeline.id + "-timeline-contours").remove();
                        _this.renderTimelineDensity(_this.timeline, _this.allEntries.find(function (d) { return d.group == group; }));
                    }
                }
            }
        });
    ...
    AdminExperimentalCharts.prototype.renderGroupCompare = function (data, id) {
        ...
        console.log(selectedGroupCompare)
...
function buildExperimentAdminAnalyticsCharts(entriesRaw) {
    ...
    function drawCharts(allEntries) {
        ...
        var updatedAllEntries = adminExperimentalCharts.handleGroups(groupChart);
