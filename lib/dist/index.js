...
var TimelineData = /** @class */ (function () {
    function TimelineData(data, colour) {
        this.data = data;
        this.colour = colour;
    }
    return TimelineData;
    ...
    AdminControlCharts.prototype.renderTimelineDensity = function (chart, data) {
        ...
        function drawContours() {
            chart.elements.content = chart.elements.contentContainer.selectAll(".timeline-container")
                ...
                .call(function (div) { return div.selectAll(".contour")
                .data(function (d) { return getDensityData(d); })
                .enter()
                .append("path")
                .attr("class", "contour")
                .attr("d", d3.geoPath())
                .attr("opacity", function (d) { return d.value * 25; }); }); }, function (update) { return update.attr("stroke", function (d) { return d.colour; })
                .attr("fill", function (d) { return d.colour; })
                .call(function (div) { return _this.interactions.timeline(div, getDensityData); }); }, function (exit) { return exit.remove(); });
...
var AdminControlTransitions = /** @class */ (function () {
    ...
    ;
    AdminControlTransitions.prototype.timeline = function (update, getDensityData) {
        update.selectAll(".contour")
            .data(function (d) { return getDensityData(d); })
            .join(function (enter) { return enter; }, function (update) { return update.attr("d", d3.geoPath())
            .attr("opacity", function (d) { return d.value * 20; }); }, function (exit) { return exit; });
    };
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroups = function (boxPlot) {
        ...
        d3.selectAll("#groups input[type=checkbox]").on("change", function (e) {
            ...
            var clickData = _this.getClickData(boxPlot.elements.contentContainer);
            updateGroupChart(boxPlot, data);
            if (boxPlot.click) {
                _this.interactions.click.appendGroupsText(boxPlot, data, clickData);
            }
            else {
                _this_1.updateHistograms(data, data.map(function (d) { return d.getUsersData(); }), data.map(function (d) { return d.group; }));
            }
    ...
    AdminExperimentalCharts.prototype.handleGroupsSort = function (boxPlot) {
        ...
        d3.select("#groups-chart #sort-by").on("click", function (e) {
            ...
            var groupClickData = _this.getClickData(boxPlot.elements.contentContainer);
            _this.renderGroupChart(boxPlot, data);
            if (boxPlot.click) {
    ...
    AdminExperimentalCharts.prototype.updateHistograms = function (data, usersData, scale) {
        ...
        if (this.histogram.click) {
            this.interactions.click.appendThresholdPercentages(this.histogram, data, this.getClickData(this.histogram.elements.contentContainer));
        }
        if (this.usersHistogram.click) {
            this.interactions.click.appendThresholdPercentages(this.usersHistogram, usersData, this.getClickData(this.usersHistogram.elements.contentContainer));
        }
    };
    ;
