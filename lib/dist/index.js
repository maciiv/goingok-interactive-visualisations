...
//exports.buildControlAuthorAnalyticsCharts = exports.buildExperimentAdminAnalyticsCharts = exports.buildControlAdminAnalyticsCharts = void 0;
//var d3 = require("d3");
var ReflectionAuthorEntryRaw = /** @class */ (function () {
    function ReflectionAuthorEntryRaw(timestamp, pseudonym, point, text) {
        this.timestamp = timestamp;
        this.pseudonym = pseudonym;
        this.point = point;
        this.text = text;
    }
    ReflectionAuthorEntryRaw.prototype.transformData = function () {
        return {
            timestamp: new Date(this.timestamp), pseudonym: this.pseudonym, point: parseInt(this.point), text: this.text
        };
    };
    return ReflectionAuthorEntryRaw;
}());
...
var ChartSummary = /** @class */ (function () {
    function ChartSummary(id, containerClass) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
...
var ChartReflectionsTime = /** @class */ (function () {
    function ChartReflectionsTime(id, containerClass, domain) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
    ...
    ChartReflectionsTime.prototype.resetTimeRange = function () {
    ...
    ChartReflectionsTime.prototype.addDays = function (date, days) {
        var result = new Date(date);
        result.setDate(result.getDate() + days);
        return result;
    };
    return ChartReflectionsTime;
}());
var AuthorControlCharts = /** @class */ (function () {
    ...
    AuthorControlCharts.prototype.processSummary = function (data) {
        var summary = [];
        var reflectionsSummary = [];
        data.forEach(function (c) {
            var reflections = Array.from(d3.rollup(c.tags, function (d) { return d.length; }, function (d) { return d.tag; }), function (_a) {
                var tag = _a[0], count = _a[1];
                return ({ tag: tag, count: count });
            });
            reflectionsSummary = reflectionsSummary.concat(reflections);
        });
        var summaryRaw = Array.from(d3.rollup(reflectionsSummary, function (d) { return d3.sum(d.map(function (c) { return c.count; })); }, function (d) { return d.tag; }), function (_a) {
            var tag = _a[0], count = _a[1];
            return ({ tag: tag, count: count });
        });
        summaryRaw.forEach(function (c) {
            summary.push({
                "tag": c.tag,
                "count": c.count,
                "percentage": Math.round(c.count * 100 / d3.sum(summaryRaw.map(function (d) { return d.count; })))
            });
        });
        return summary;
    };
    AuthorControlCharts.prototype.renderSummary = function (chart, data) {
        chart.elements.contentContainer.selectAll(".summary-bar")
            .data(data)
            .join(function (enter) { return enter.append("rect")
            .attr("class", function (d) { return "summary-bar " + d.tag.toLowerCase(); })
            .attr("id", function (d) { return d.tag.toLowerCase() + "-summary"; })
            .attr("x", function (d, i) { return chart.x.scale(i === 0 ? 0.1 : getTotalPercentage(data, i)); })
            .attr("width", chart.x.scale(0))
            .attr("height", chart.y.scale(0))
            .call(function (update) { return update.transition()
            .duration(750)
            .attr("width", function (d) { return chart.x.scale(d.percentage); }); }); }, function (update) { return update; }, function (exit) { return exit; });
        function getTotalPercentage(data, i) {
            var result = 0;
            for (var x = 0; x < i; x++) {
        ...
        chart.elements.contentContainer.selectAll(".summary-percentage")
            ...
            .attr("x", function (d, i) { return chart.x.scale(i === 0 ? d.percentage / 2 : getTotalPercentage(data, i) + (d.percentage / 2)); })
            .attr("y", chart.y.scale(0.5))
            .text(function (d) { return d.percentage + "%"; }); }, function (update) { return update; }, function (exit) { return exit; });
        chart.elements.svg.selectAll(".summary-tag")
            .data(data)
            .join(function (enter) { return enter.append("text")
            .attr("class", "summary-tag")
            .attr("x", function (d, i) { return chart.x.scale(i === 0 ? d.percentage / 2 : getTotalPercentage(data, i) + (d.percentage / 2)) + 5; })
            .attr("y", chart.y.scale(0) + 10)
            .text(function (d) { return d.tag; }); }, function (update) { return update; }, function (exit) { return exit; });
        return chart;
    };
    AuthorControlCharts.prototype.renderReflectionsTimeline = function (chart, data) {
        var _this = this;
        ...
        chart.elements.contentContainer.selectAll(".reflection-group")
            .data(d3.sort(data, function (d) { return d.timestamp; }))
            .join(function (enter) { return enter.append("g")
            .attr("class", "reflection-group")
            .attr("transform", "translate(" + chart.width / 2 + ", " + chart.height / 2 + ")")
            .call(function (enter) { return enter.selectAll(".summary-pie")
            .data(function (d) { return pie(Array.from(d3.rollup(d.tags, function (d) { return d.length; }, function (d) { return d.tag; }), function (_a) {
            var tag = _a[0], count = _a[1];
            return ({ tag: tag, count: count });
        })); })
            .enter()
            .append("path")
            .attr("class", function (d) { return "summary-pie " + d.data.tag.toLowerCase(); })
            .attr("d", d3.arc()
            .innerRadius(15)
            .outerRadius(30)); })
            ...
            .duration(750)
            .attr("transform", function (d) { return "translate(" + chart.x.scale(d.timestamp) + ", " + chart.y.scale(d.point) + ")"; }); }); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        chart.elements.content = chart.elements.contentContainer.selectAll(".reflection-group .summary-pie");
        //Enable tooltip       
        _this.interactions.tooltip.enableTooltip(chart, onMouseover, onMouseout);
        function onMouseover(e, d) {
            var parentData = d3.select(e.path[1]).datum();
            var isRef = d3.select(this).attr("class").includes("ref");
            _this.interactions.tooltip.appendTooltipContainer(chart);
            var tooltipBox = _this.interactions.tooltip.appendTooltipText(chart, isRef ? d.timestamp.toDateString() : d.data.tag, isRef ? [new TooltipValues("Point", d.point)] :
        ...
        function zoomed(e) {
            ...
            chart.x.scale.rangeRound(newChartRange);
            var newTimeLink = d3.line()
                .x(function (d) { return chart.x.scale(d.timestamp); })
                .y(function (d) { return chart.y.scale(d.point); });
            chart.elements.contentContainer.selectAll(".reflection-group")
                .attr("transform", function (d) { return "translate(" + chart.x.scale(d.timestamp) + ", " + chart.y.scale(d.point) + ")"; });
            chart.elements.contentContainer.selectAll(".reflection-link-time")
                .attr("d", function (d) { return newTimeLink(d); });
            chart.elements.contentContainer.selectAll(".click-container")
                .attr("transform", function (d) { return "translate(" + chart.x.scale(d.timestamp) + ", " + chart.y.scale(d.point) + ")"; });
            chart.x.axis.ticks(newChartRange[1] / 75);
            chart.elements.xAxis.call(chart.x.axis);
    ...
    AuthorControlCharts.prototype.preProcessNetwork = function (chart, entries, data) {
        var reflections = entries.map(function (d) { return { "timestamp": d.timestamp, "tagsLength": d.tags.length }; });
        var startIndex = 0;
        reflections.forEach(function (c) {
            var refTag = { "tag": "ref", "phrase": c.timestamp.toDateString(), "start_index": startIndex, "end_index": startIndex + c.tagsLength - 1, "fx": chart.x.scale(c.timestamp) };
            if (!data.tags.map(function (d) { return d.phrase; }).includes(refTag.phrase)) {
                var newTagsL = data.tags.push(refTag);
        ...
        chart.simulation = d3.forceSimulation(data.tags)
    ...
    AuthorControlCharts.prototype.renderNetwork = function (chart, data) {
        ...
        function ticked(chart) {
            chart.elements.contentContainer.selectAll(".network-link")
                ...
                .attr("y2", function (d) { return d.target.y; }); }); }, function (update) { return update; }, function (exit) { return exit.remove(); });
            chart.elements.contentContainer.selectAll(".network-node-group")
                ...
                .attr("class", function (d) { return "network-node " + d.tag.toLowerCase(); }); })
                ...
                .call(function (enter) { return enter.transition()
                .duration(750)
                .attr("transform", function (d) { return "translate(" + d.x + ", " + d.y + ")"; }); }); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        }
        function applyForce(d, chart) {
            if (chart.simulation === undefined || chart.simulation.nodes() !== d.tags) {
                chart.simulation = d3.forceSimulation(d.tags)
                    .force("link", d3.forceLink()
                    .id(function (d) { return d.index; })
                    .distance(100)
                    .links(d.links))
    ...
    AuthorControlCharts.prototype.renderReflections = function (data) {
        ...
        d3.select("#reflections .reflections-tab")
            ...
            .attr("class", "mb-0")
            .html(function (d) { return "<i>" + d.timestamp.toDateString() + " | Point: " + d.point; }); })
            .call(function (div) { return div.append("p")
            .html(function (d) { return _this.processReflectionsText(d.tags, d.words); }); });
    };
    AuthorControlCharts.prototype.processReflectionsText = function (tags, words) {
        var html = "";
        for (var i = 0; i <= words.length; i++) {
            if (words[i] === undefined)
                continue;
            var currentWord = words[i];
            var isOpenTag = tags.find(function (c) { return c.start_index === i; });
            var isCloseTag = tags.find(function (c) { return c.end_index === i; });
    ...
    AuthorControlCharts.prototype.handleTimelineButtons = function (chart, entries, data, func) {
        var _this = this;
        d3.select("#" + chart.id + " #timeline-plot").on("click", func != undefined ? function (e) { return func(e); } : function (e) {
            var selectedOption = e.target.control.value;
...
function buildControlAuthorAnalyticsCharts(analyticsRaw, networkRaw) {
    return __awaiter(this, void 0, void 0, function () {
        function drawCharts(entries, network) {
            return __awaiter(this, void 0, void 0, function () {
                var authorControlCharts, summaryData, summaryChart, reflectionTimelineChart, processedNetworkData;
                return __generator(this, function (_a) {
                    authorControlCharts = new AuthorControlCharts();
                    summaryData = authorControlCharts.processSummary(entries);
                    summaryChart = new ChartSummary("summary", "chart-container-summary");
                    authorControlCharts.renderSummary(summaryChart, summaryData);
                    reflectionTimelineChart = new ChartReflectionsTime("timeline", "chart-container-timeline", entries.map(function (d) { return d.timestamp; }));
                    authorControlCharts.renderReflectionsTimeline(reflectionTimelineChart, entries);
                    processedNetworkData = authorControlCharts.preProcessNetwork(reflectionTimelineChart, entries, network);
                    ...
                    d3.select("#timeline .card-title button")
                        ...
                        if (!reflectionTimelineChart.elements.contentContainer.select(".reflection-group").empty()) {
                            ...
                            var showDataHelp = authorControlCharts.help.helpPopover(reflectionTimelineChart.elements.contentContainer.select(".reflection-group"), reflectionTimelineChart.id + "-help-data", "<u><i>hover</i></u> me for information on demand");
                            if (showDataHelp) {
        ...
        var loading, entries, network;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    loading = new Loading();
                    entries = analyticsRaw.map(function (d) { return { "timestamp": new Date(d.timestamp), "pseudonym": d.pseudonym, "point": d.point, "text": d.text, "tags": d.tags, "words": d.words }; });
                    network = { "tags": networkRaw.tags, "links": networkRaw.links };
                    return [4 /*yield*/, drawCharts(entries, network)];
                case 1:
                    _a.sent();
                    loading.isLoading = false;
                    loading.removeDiv();
