...
var AdminControlCharts = /** @class */ (function () {
    ...
    AdminControlCharts.prototype.renderUserStatistics = function (card, data, thresholds, pseudonym) {
        ...
        d3.select("#user-statistics .card-subtitle")
            .html("The user " + userData.value[d3.minIndex(userData.value.map(function (d) { return d.point; }))].pseudonym + " is the most distressed, while\n                the user " + userData.value[d3.maxIndex(userData.value.map(function (d) { return d.point; }))].pseudonym + " in the most soaring");
        function getBinName(data) {
        ...
        card.selectAll("div")
            ...
            .html(function (d) { return "<b>" + getBinName(d) + "</b>"; }); }); })
            .call(function (div) { return div.append("div")
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroups = function (boxPlot) {
        ...
        d3.selectAll("#groups input[type=checkbox]").on("change", function (e) {
            ...
            if (boxPlot.click) {
                ...
                _this.histogram.x.scale.domain(data.map(function (r) { return r.group; }));
                _this.usersHistogram.x.scale.domain(data.map(function (r) { return r.group; }));
    ...
    AdminExperimentalCharts.prototype.handleGroupsColours = function (boxPlot) {
        ...
        d3.selectAll("#groups input[type=color]").on("change", function (e) {
            ...
            _this.renderHistogram(_this.histogram, data);
            _this.renderHistogram(_this.usersHistogram, data);
            if (boxPlot.click) {
        ...
        d3.select("#groups-chart #sort-by").on("click", function (e) {
            ...
            if (boxPlot.click) {
                ...
                if (_this.histogram.click) {
                    _this.interactions.click.appendThresholdPercentages(_this.histogram, data, histogramClickData);
                }
    ...
    AdminExperimentalCharts.prototype.renderGroupChart = function (chart, data) {
        ...
        function onClick(e, d) {
            ...
            _this.usersHistogram.x.scale.domain(data.filter(function (c) { return c.group == d.group; }).map(function (d) { return d.group; }));
            var usersData = d.getUsersData();
            _this.renderHistogram(_this.usersHistogram, [usersData]);
            _this.interactions.axisSeries(_this.usersHistogram, [usersData]);
            _this.histogram.x.scale.domain(data.filter(function (c) { return c.group == d.group; }).map(function (d) { return d.group; }));
            _this.renderHistogram(_this.histogram, [d]);
            _this.interactions.axisSeries(_this.histogram, [d]);
            _this.htmlContainers.removeHelp(chart);
        }
        function restoreCharts() {
            _this.histogram.x.scale.domain(data.map(function (d) { return d.group; }));
            _this.renderHistogram(_this.histogram, data);
            var usersData = data.map(function (d) { return d.getUsersData(); });
            _this.usersHistogram.x.scale.domain(data.map(function (d) { return d.group; }));
            _this.renderHistogram(_this.usersHistogram, usersData);
            _this.interactions.axisSeries(_this.histogram, data);
            _this.interactions.axisSeries(_this.usersHistogram, usersData);
        }
    ...
    AdminExperimentalCharts.prototype.renderTimelineScatter = function (chart, zoomChart, data) {
        ...
        function onClick(e, d) {
            ...
            _this.renderUserStatistics(userStatisticsCard, data, _this.usersHistogram.elements.getThresholdsValues(_this.usersHistogram), d.pseudonym);
            _this.htmlContainers.removeHelp(chart);
            //Scroll
    ...
    ;
    AdminExperimentalCharts.prototype.renderUserStatistics = function (card, data, thresholds, pseudonym) {
        _super.prototype.renderUserStatistics.call(this, card, data, this.usersHistogram.elements.getThresholdsValues(this.usersHistogram), pseudonym);
        var _this = this;
        card.selectAll("a")
...
function buildExperimentAdminAnalyticsCharts(entriesRaw) {
    ...
    function drawCharts(allEntries) {
        ...
        adminExperimentalCharts.htmlContainers.appendCard(adminExperimentalCharts.htmlContainers.histogram, "Reflections by group - Histogram", undefined, true);
        ...
        var timelineCard = adminExperimentalCharts.htmlContainers.appendCard(adminExperimentalCharts.htmlContainers.timeline, "Reflections by group vs time", undefined, true);
        adminExperimentalCharts.timeline = new ChartTime("group-timeline", d3.extent(data[0].value.map(function (d) { return d.timestamp; })));
        adminExperimentalCharts.renderTimelineDensity(adminExperimentalCharts.timeline, data[0]);
        var timelineZoomChart = new ChartTimeZoom(adminExperimentalCharts.timeline, d3.extent(data[0].value.map(function (d) { return d.timestamp; })));
        ...
        adminExperimentalCharts.htmlContainers.appendCard(adminExperimentalCharts.htmlContainers.userHistogram, "All reflections by users and group - Histogram", undefined, true);
        var usersData = data.map(function (d) { return d.getUsersData(); });
