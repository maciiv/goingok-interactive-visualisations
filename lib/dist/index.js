...
var SummaryChart = /** @class */ (function () {
    function SummaryChart(id, containerClass) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
...
var NetworkChart = /** @class */ (function () {
    function NetworkChart(id, containerClass, domain) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
...
var MultilevelTimeChart = /** @class */ (function () {
    function MultilevelTimeChart(id, containerClass, domain, levels) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
...
var AuthorControlCharts = /** @class */ (function () {
    ...
    AuthorControlCharts.prototype.renderMultilevelTimeline = function (chart, data, summary) {
        var reflectionsSummary = [];
        var allTags = summary.map(function (d) { return d.tag; });
        data.forEach(function (c) {
            allTags.forEach(function (d) {
                reflectionsSummary.push({
                    tag: d,
                    count: c.tags.filter(function (r) { return r.tag === d; }).length,
                    percentage: Math.round(c.tags.filter(function (r) { return r.tag === d; }).length * 100 / c.tags.length),
                    timestamp: c.timestamp
                });
            });
        });
        var timelineLine = d3.area()
            .x(function (d) { return chart.x.scale(d.timestamp); })
            .y(function (d) { return chart.y.scale(d.percentage); });
        chart.elements.contentContainer.selectAll(".multilevel-group")
            .data(allTags)
            .join(function (enter) { return enter.append("g")
            .attr("class", "multilevel-group")
            .call(function (enter) { return enter.append("path")
            .attr("class", function (d) { return "multilevel-area " + d.toLowerCase(); })
            .datum(function (d) { return d3.sort(reflectionsSummary.filter(function (c) { return c.tag === d; }), function (d) { return d.timestamp; }); })
            .attr("d", d3.area()
            .x(function (d) { return chart.x.scale(d.timestamp); })
            .y0(function (d) { return chart.y.scale(0); })
            .y1(function (d) { return chart.y.scale(d.percentage); })); }); });
        return chart;
    };
...
function buildControlAuthorAnalyticsCharts(entriesRaw) {
    return __awaiter(this, void 0, void 0, function () {
        function drawCharts(entries) {
            return __awaiter(this, void 0, void 0, function () {
                var authorControlCharts, summaryData, summaryChart, networkChart, multilevelTimeChart;
                return __generator(this, function (_a) {
                    authorControlCharts = new AuthorControlCharts();
                    summaryData = authorControlCharts.processSummary(entries);
                    summaryChart = new SummaryChart("summary", "chart-container-summary");
                    authorControlCharts.renderSummary(summaryChart, summaryData);
                    networkChart = new NetworkChart("timeline", "chart-container-network", entries.map(function (d) { return d.timestamp; }));
                    authorControlCharts.renderNetwork(networkChart, entries);
                    multilevelTimeChart = new MultilevelTimeChart("timeline", "chart-container-timeline", entries.map(function (d) { return d.timestamp; }), summaryData.length);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    loading = new Loading();
                    entries = entriesRaw.map(function (d) { return { "timestamp": new Date(d.timestamp), "pseudonym": d.pseudonym, "point": d.point, "text": d.text, "tags": d.tags, "words": d.words }; });
                    return [4 /*yield*/, drawCharts(entries)];
                case 1:
                    _a.sent();
                    loading.isLoading = false;
