...
var AuthorControlCharts = /** @class */ (function () {
    ...
    AuthorControlCharts.prototype.renderTotals = function (authorData, data) {
        var point = d3.select("#point-avg .card-title span").datum();
        d3.select("#point-avg .card-title span")
            .datum(d3.mean(authorData.map(function (d) { return d.point; })))
            .transition()
            .duration(1000)
            .tween("html", function () {
            var oldPoint = point == undefined ? 0 : point;
            var newPoint = d3.mean(authorData.map(function (d) { return d.point; }));
            return function (t) {
        ...
        var refs = d3.select("#ref-total .card-title span").datum();
        d3.select("#ref-total .card-title span")
            .datum(d3.count(authorData.map(function (d) { return d.timestamp; })))
            .transition()
            .duration(1000)
            .tween("html", function () {
            var oldRefs = refs == undefined ? 0 : refs;
            var newRefs = d3.count(authorData.map(function (d) { return d.timestamp; }));
            return function (t) {
                if (oldRefs < newRefs) {
                    this.innerHTML = (oldRefs + Math.round(t * (newRefs - oldRefs))).toString();
                }
        ...
        var you = d3.mean(authorData.map(function (d) { return d.point; })) < 30 ? "Distressed" : d3.mean(authorData.map(function (d) { return d.point; })) > 70 ? "Soaring" : "GoingOK";
        d3.select("#you .card-title span")
    ...
    AuthorControlCharts.prototype.renderTimeline = function (chart, zoomChart, authorData, data) {
        var _this = this;
        d3.select("#" + chart.id + " .card-subtitle")
            .classed("instructions", data.length <= 1)
            .classed("text-muted", data.length != 1)
            .html(data.length != 1 ? "Your oldest reflection was on " + d3.min(authorData.map(function (d) { return d.timestamp; })).toDateString() + ", while\n                the newest reflection was on " + d3.max(authorData.map(function (d) { return d.timestamp; })).toDateString() :
        ...
        //Draw author line
        chart.elements.contentContainer.append("path")
            .datum(d3.sort(authorData, function (d) { return d.timestamp; }))
        ...
        var area = d3.area()
            ...
            .y1(function (d) { return chart.y.scale(d.point); })
            .curve(d3.curveMonotoneX);
        //Draw group area
        chart.elements.contentContainer.append("path")
        ...
        //Draw circles
        chart.elements.contentContainer.selectAll("circle")
            .data(d3.sort(authorData, function (d) { return d.timestamp; }))
            .join(function (enter) { return enter.append("circle")
            .attr("class", "circle")
            .attr("cx", function (d) { return chart.x.scale(d.timestamp); })
            .attr("cy", function (d) { return chart.y.scale(d.point); })
            .attr("r", 5)
            .style("stroke", "#3399CC")
            .style("fill", "#3399CC"); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        chart.elements.content = chart.elements.contentContainer.selectAll(".circle");
        //Enable tooltip       
        _this.interactions.tooltip.enableTooltip(chart, onMouseover, onMouseout);
        function onMouseover(e, d) {
        ...
        chart.elements.zoomFocus.selectAll(".zoom-timeline-content-container")
            ...
            .attr("r", 2); }, function (update) { return update; }, function (exit) { return exit.remove(); });
        var zoomLine = d3.line()
            .x(function (d) { return zoomChart.x.scale(d.timestamp); })
            .y(function (d) { return zoomChart.y.scale(d.point); })
            .curve(d3.curveMonotoneX);
        var zoomArea = d3.area()
            .x(function (d) { return zoomChart.x.scale(d.timestamp); })
            .y0(function (d) { return zoomChart.y.scale(0); })
            .y1(function (d) { return zoomChart.y.scale(d.point); })
            .curve(d3.curveMonotoneX);
        chart.elements.zoomSVG.append("path")
            .datum(d3.sort(data, function (d) { return d.timestamp; }))
            .attr("class", "group-area")
            .attr("d", function (d) { return zoomArea(d); });
        chart.elements.zoomSVG.append("path")
            .datum(d3.sort(authorData, function (d) { return d.timestamp; }))
            .attr("class", "author-line")
            .attr("d", function (d) { return zoomLine(d); });
        chart.elements.zoomSVG.selectAll(".zoom-timeline-container")
            .data(authorData)
            .join(function (enter) { return enter.append("circle")
        ...
        function zoomed(e) {
            ...
            chart.elements.zoomFocus.selectAll(".zoom-content")
                .attr("cx", function (d) { return zoomChart.x.scale(d.timestamp); });
            chart.elements.contentContainer.selectAll(".author-line")
                .attr("d", function (d) { return newLine(d); });
            chart.elements.contentContainer.selectAll(".group-area")
                .attr("d", function (d) { return newArea(d); });
            chart.elements.contentContainer.selectAll(".click-container")
                .attr("transform", function (d) { return "translate(" + chart.x.scale(d.timestamp) + ", " + chart.y.scale(d.point) + ")"; });
            chart.x.axis.ticks(newChartRange[1] / 75);
            chart.elements.xAxis.call(chart.x.axis);
...
function buildControlAuthorAnalyticsCharts(pseudonym, entriesRaw) {
    return __awaiter(this, void 0, void 0, function () {
        function drawCharts(authorEntries, entries) {
            return __awaiter(this, void 0, void 0, function () {
                var authorControlCharts, timelineChart, timelineZoomChart;
                return __generator(this, function (_a) {
                    authorControlCharts = new AuthorControlCharts();
                    authorControlCharts.renderTotals(authorEntries, entries);
                    timelineChart = new ChartTime("timeline", d3.extent(entries.map(function (d) { return d.timestamp; })));
                    timelineZoomChart = new ChartTimeZoom(timelineChart, d3.extent(entries.map(function (d) { return d.timestamp; })));
                    authorControlCharts.renderTimeline(timelineChart, timelineZoomChart, authorEntries, entries);
                    return [2 /*return*/];
                });
            });
        }
        var loading, rawData, entries, authorEntries;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    loading = new Loading();
                    rawData = entriesRaw.map(function (d) { return new ReflectionAuthorEntryRaw(d.timestamp, d.pseudonym, d.point, d.text); });
                    entries = rawData.map(function (d) { return d.transformData(); });
                    authorEntries = entries.filter(function (d) { return d.pseudonym === pseudonym; });
                    return [4 /*yield*/, drawCharts(authorEntries, entries)];
                case 1:
                    _a.sent();
                    loading.isLoading = false;
                    loading.removeDiv();
