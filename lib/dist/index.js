...
var Chart = /** @class */ (function () {
    function Chart() {
        this.renderElements = new ChartRenderElements();
        this.padding = { xAxis: 50, yAxis: 75, top: 25, right: 0 };
    }
    Chart.prototype.basicChart = function (id) {
    ...
    ;
    Chart.prototype.setChart = function (id, data) {
        var result = this.basicChart(id);
        result.x = new ChartAxis("Group Code", data.map(function (r) { return r.group; }), [0, result.width - result.padding.yAxis - result.padding.right], "bottom");
        return result;
    };
    ;
    Chart.prototype.setTimelineChart = function (id, data) {
        var result = this.basicChart(id);
        result.padding.xAxis = result.padding.xAxis + 25;
        result.padding.top = 5;
        result.x = new ChartAxis("Time", d3.extent(data.map(function (r) { return r.timestamp; })), [0, result.width - result.padding.yAxis], "bottom");
        return result;
    };
    ;
    Chart.prototype.setTimelineZoomChart = function (chart, data) {
        var result = new Chart();
        result.x = new ChartAxis("", d3.extent(data.map(function (r) { return r.timestamp; })), [0, chart.width - chart.padding.yAxis - 5], "bottom");
        result.y = new ChartAxis("", [0, 100], [25, 0], "left");
        return result;
    };
    return Chart;
}());
var ChartAxis = /** @class */ (function () {
    function ChartAxis(label, domain, range, position) {
        this.label = label;
        ...
        if (typeof domain == "number") {
            ...
            this.axis.tickValues([0, 25, 50, 75, 100])
                .tickFormat(function (d, i) { return labels_1.get(d); });
        }
        ;
    }
    ;
    ChartAxis.prototype.setThresholdAxis = function (chart, tDistressed, tSoaring) {
        return d3.axisRight(chart.y.scale)
            .tickValues([tDistressed, tSoaring])
            .tickFormat(function (d) { return d == tDistressed ? "Distressed" : d == tSoaring ? "Soaring" : ""; });
    };
    ;
    ChartAxis.prototype.positionAxis = function (position, scale) {
        if (position == "left") {
...
var ChartRenderElements = /** @class */ (function () {
    function ChartRenderElements() {
    }
    ChartRenderElements.prototype.preRender = function (chart) {
    ...
    ChartRenderElements.prototype.appendSVG = function (chart) {
    ...
    ;
    ChartRenderElements.prototype.appendContentContainer = function (chart) {
    ...
    ;
    ChartRenderElements.prototype.appendXAxis = function (chart) {
    ...
    ;
    ChartRenderElements.prototype.appendXAxisLabel = function (chart) {
        return chart.renderElements.svg.append("g")
    ...
    ;
    ChartRenderElements.prototype.appendYAxis = function (chart) {
        return chart.renderElements.svg.append("g")
            .attr("transform", "translate(" + chart.padding.yAxis + ", " + chart.padding.top + ")")
            .attr("class", "y-axis")
            .call(chart.y.axis);
    ...
    ;
    ChartRenderElements.prototype.appendYAxisLabel = function (chart) {
        return chart.renderElements.svg.append("g")
    ...
    ;
    ChartRenderElements.prototype.appendThresholdAxis = function (chart, axis) {
    ...
    ;
    ChartRenderElements.prototype.appendThresholdLabel = function (chart) {
    ...
    ;
    ChartRenderElements.prototype.appendThresholdIndicators = function (chart, thresholds) {
...
function buildControlAdminAnalyticsCharts(entries) {
    ...
    function drawCharts(entries) {
        ...
        var groupChart = new Chart().setChart("groups-chart", data);
...
function buildExperimentAdminAnalyticsCharts(entries) {
    ...
    function drawCharts(allEntries) {
        ...
        function renderGroupChart(chart, data) {
            ...
            function onMouseover(e, d) {
                ...
                function xTooltip(x, tooltipBox) {
                    ...
                    var xTooltip = xScale(x) + xScale.bandwidth();
                ...
                function yTooltip(y, tooltipBox) {
                    ...
                    var yTooltip = yScale(y) - (tooltipBox.node().getBBox().height / 2);
        ...
        function groupTimeline(data) {
            var timelineChart = chartFunctions.setTimelineChart("group-timeline", data.value);
            chartFunctions.preRender(timelineChart);
            renderTimelineDensity(timelineChart, data.value);
            var timelineZoomChart = chartFunctions.setTimelineZoomChart(timelineChart, data.value);
            ...
            function renderTimelineDensity(chart, data) {
                ...
                //Create density data
                var densityData = createDensityData(xScale);
                function createDensityData(xScale) {
                ...
                function zoomed(e) {
                    ...
                    var newDensityData = createDensityData(xScale);
            ...
            function renderTimelineScatter(chart, zoomChart, data, stats) {
                ...
                var zoomXScale = zoomChart.x.scale;
                var zoomYScale = zoomChart.y.scale;
                //Draw in zoom bar
                chart.renderElements.zoomSVG.selectAll(chart.id + "zoom-bar-content")
                    ...
                    .attr("r", 2)
                    .attr("cx", function (d) { return zoomXScale(d.timestamp); })
                    .attr("cy", function (d) { return zoomYScale(d.point); });
                ...
                function zoomed(e) {
                    var newChartRange = [0, chart.width - chart.padding.yAxis].map(function (d) { return e.transform.applyX(d); });
                    xScale.rangeRound(newChartRange);
                    zoomXScale.rangeRound([0, chart.width - chart.padding.yAxis - 5].map(function (d) { return e.transform.invertX(d); }));
                    var newLine = d3.line()
                        .x(function (d) { return xScale(d.timestamp); })
                        .y(function (d) { return yScale(d.point); });
                    ...
                    chart.renderElements.contentContainer.selectAll(".click-container")
                        .attr("transform", function (d) { return "translate(" + xScale(d.timestamp) + ", " + yScale(d.point) + ")"; });
        ...
        function groupViolinChart(data, groups) {
            ...
            chartFunctions.preRender(violinChart);
            ...
            chartFunctions.preRender(violinUsersChart);
        ...
        function renderViolin(chart, data) {
            ...
            function dragViolinThresholds(chart, data, tDistressed, tSoaring) {
                ...
                function draggingSoaring(e, d) {
                    ...
                    thresholdAxis.tickValues([tDistressed, yScale.invert(e.y)])
                        .tickFormat(function (d) { return d == tDistressed ? "Distressed" : d == yScale.invert(e.y) ? "Soaring" : ""; });
                    ...
                    var positionY = yScale(tSoaring) + 25;
                ...
                function draggingDistressed(e, d) {
                    ...
                    thresholdAxis.tickValues([yScale.invert(e.y), tSoaring])
                        .tickFormat(function (d) { return d == yScale.invert(e.y) ? "Distressed" : d == tSoaring ? "Soaring" : ""; });
                    chart.renderElements.contentContainer.selectAll(".threshold-axis")
                        .call(thresholdAxis);
            ...
            function drawViolin(chart, data, tDistressed, tSoaring) {
                ...
                binContainerEnter.append("path")
                    ...
                    .y(function (d, i) { return yScale(i == 0 ? 0 : i == 1 ? 50 : 100); })
...
var chartFunctions = {
    ...
    setChart: function (id, data) {
        var result = chartFunctions.basicChart(id);
    ...
    setTimelineChart: function (id, data) {
        var result = chartFunctions.basicChart(id);
        result.padding.xAxis = result.padding.xAxis + 25;
        result.padding.top = 5;
    ...
    setTimelineZoomChart: function (chart, data) {
        var result = new Chart();
    ...
    data: {
        processEntries: function (entries) {
            ...
            entries.forEach(function (c) {
                ...
                result.push({
                    value: c.value.map(function (r) { return { timestamp: new Date(r.timestamp), point: r.point, pseudonym: r.pseudonym, text: r.text }; }),
                    group: c.group,
    ...
    axis: {
        ...
        setValuesAxis: function (label, values, range, position, isGoingOk) {
            ...
            result.axis = this.positionAxis(position, result.scale);
            if (isGoingOk) {
                var labels_2 = new Map();
                labels_2.set(0, "distressed");
                labels_2.set(50, "going ok");
                labels_2.set(100, "soaring");
                result.axis.tickValues([0, 25, 50, 75, 100])
                    .tickFormat(function (d, i) { return labels_2.get(d); });
            }
            return result;
        },
...
var adminAnalyticsCharts = {
    renderGroupChart: function (chart, data) {
        //Set scale types
        var xScale = chart.x.scale;
        var yScale = chart.y.scale;
        //Select existing minMax lines
