...
//exports.buildExperimentAuthorAnalyticsCharts = exports.buildControlAuthorAnalyticsCharts = exports.buildExperimentAdminAnalyticsCharts = exports.buildControlAdminAnalyticsCharts = void 0;
//var d3 = require("d3");
var AdminAnalyticsDataRaw = /** @class */ (function () {
    function AdminAnalyticsDataRaw(group, value, createDate) {
        this.group = group;
        this.value = value;
        this.createDate = createDate;
    }
    AdminAnalyticsDataRaw.prototype.transformData = function () {
        return new AdminAnalyticsData(this.group, this.value.map(function (d) {
            return {
    ...
    return AdminAnalyticsDataRaw;
}());
var AdminAnalyticsData = /** @class */ (function () {
    function AdminAnalyticsData(group, value, createDate, colour, selected) {
    ...
    AdminAnalyticsData.prototype.getUsersData = function () {
...
var AdminAnalyticsDataStats = /** @class */ (function (_super) {
    __extends(AdminAnalyticsDataStats, _super);
    function AdminAnalyticsDataStats(entries) {
    ...
    ;
    AdminAnalyticsDataStats.prototype.roundDecimal = function (value) {
        var p = d3.precisionFixed(0.1);
        var f = d3.format("." + p + "f");
        return f(value);
    };
    ;
    AdminAnalyticsDataStats.prototype.getStat = function (stat) {
    ...
    return AdminAnalyticsDataStats;
}(AdminAnalyticsData));
...
var ChartTime = /** @class */ (function () {
    function ChartTime(id, domain, chartPadding) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " .chart-container").node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
...
var Transitions = /** @class */ (function () {
    ...
    Transitions.prototype.axisSeries = function (chart, data) {
    ...
    ;
    Transitions.prototype.axisTime = function (chart, data) {
    ...
    ;
    Transitions.prototype.axisLinear = function (chart) {
...
var AdminControlTransitions = /** @class */ (function (_super) {
    __extends(AdminControlTransitions, _super);
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroupsSort = function () {
        var _this = this;
        d3.select("#sort .btn-group-toggle").on("click", function (e) {
            var selectedOption = e.target.control.value;
    ...
    AdminExperimentalCharts.prototype.getUpdatedData = function () {
        return d3.filter(this.allEntries, function (d) { return d.selected; }).map(function (d) { return new AdminAnalyticsDataStats(d); });
    };
    ;
...
var ClickAdmin = /** @class */ (function (_super) {
    __extends(ClickAdmin, _super);
    function ClickAdmin() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    ClickAdmin.prototype.appendScatterText = function (chart, d, title, values) {
        if (values === void 0) { values = null; }
    ...
    ;
    ClickAdmin.prototype.positionClickContainer = function (chart, box, text, d) {
    ...
    ;
    ClickAdmin.prototype.appendGroupsText = function (chart, data, clickData) {
        var _this_1 = this;
    ...
    ;
    ClickAdmin.prototype.appendThresholdPercentages = function (chart, data, clickData) {
        var _this_1 = this;
        var thresholds = chart.elements.getThresholdsValues(chart);
        var tDistressed = thresholds[0];
        var tSoaring = thresholds[1];
    ...
    ;
    ClickAdmin.prototype.comparativeText = function (textData) {
...
var ChartTimeNetwork = /** @class */ (function (_super) {
    __extends(ChartTimeNetwork, _super);
    function ChartTimeNetwork(id, domain, chartPadding) {
        var _this_1 = _super.call(this, id, domain, chartPadding) || this;
        _this_1.x.scale.domain([_this_1.addDays(d3.min(domain), -30), _this_1.addDays(d3.max(domain), 30)]);
        _this_1.elements.xAxis.call(_this_1.x.axis);
        return _this_1;
    }
    ChartTimeNetwork.prototype.addDays = function (date, days) {
        var result = new Date(date);
        result.setDate(result.getDate() + days);
        return result;
    };
    return ChartTimeNetwork;
}(ChartTime));
// Basic class for network chart timeline
var ChartNetwork = /** @class */ (function () {
    function ChartNetwork(id, containerClass, domain) {
        this.id = id;
        var containerDimensions = d3.select("#" + id + " ." + containerClass).node().getBoundingClientRect();
        this.width = containerDimensions.width;
        this.height = containerDimensions.height;
    ...
    ChartNetwork.prototype.addDays = function (date, days) {
        var result = new Date(date);
        result.setDate(result.getDate() + days);
        return result;
    };
    ChartNetwork.prototype.resetZoomRange = function () {
...
var AuthorControlCharts = /** @class */ (function () {
    function AuthorControlCharts() {
        this.help = new Help();
        this.interactions = new AuthorControlInteractions();
    }
    AuthorControlCharts.prototype.preloadTags = function (entries, enable) {
        if (enable === void 0) { enable = false; }
        var allTags = [];
        entries.forEach(function (c) {
            allTags = allTags.concat(c.tags.map(function (d) { {
                return { "tag": d.tag, "colour": d.colour };
            } }));
        });
        var groupTags = Array.from(d3.rollup(allTags, function (d) { return d.map(function (r) { return r.colour; })[0]; }, function (d) { return d.tag; }), function (_a) {
            var tag = _a[0], colour = _a[1];
            return ({ tag: tag, colour: colour });
        });
        var uniqueTags = groupTags.map(function (d) { return { "tag": d.tag, "colour": d.colour, "selected": true }; });
        d3.select("#tags").selectAll("li")
            ...
            .append("input")
            .attr("id", function (d) { return "colour-" + d.tag; })
            .attr("type", "color")
            .attr("value", function (d) { return d.colour; })
            .property("disabled", !enable); });
        return uniqueTags;
    };
    AuthorControlCharts.prototype.processNetworkData = function (chart, entries) {
        var networkData = { "nodes": [], "links": [] };
        entries.forEach(function (c) {
            var tags = c.tags.map(function (d) { return __assign({}, d); });
            networkData.nodes = networkData.nodes.concat(tags);
            var refTag = { "tag": "ref", "phrase": c.timestamp.toDateString(), "colour": "#f2f2f2", "fx": chart.x.scale(c.timestamp) };
            networkData.nodes.push(refTag);
            c.matrix.forEach(function (r, i) {
    ...
    AuthorControlCharts.prototype.processSimulation = function (chart, data) {
    ...
    AuthorControlCharts.prototype.processTimelineSimulation = function (chart, centerX, centerY, nodes) {
    ...
    AuthorControlCharts.prototype.renderTimeline = function (chart, data) {
        var _this = this;
        ...
        if (chart.elements.contentContainer.select(".hardline").empty()) {
            chart.elements.contentContainer.append("path")
                .datum(d3.sort(data, function (d) { return d.timestamp; }))
                .attr("class", "hardline")
                .attr("d", function (d) { return hardLine(d); });
        }
        chart.elements.contentContainer.selectAll(".circle-tag-container")
            .data(data)
            .join(function (enter) { return enter.append("g")
            .attr("class", "circle-tag-container")
            .call(function (enter) { return enter.append("circle")
            .attr("class", "circle")
            .attr("r", 5)
            .style("fill", "#999999")
            .style("stroke", "#999999"); })
            .call(function (enter) { return renderTimelineNetwork(enter); })
            .call(function (enter) { return enter.transition()
            .duration(750)
            .attr("transform", function (d) { return "translate (" + chart.x.scale(d.timestamp) + ", " + chart.y.scale(d.point) + ")"; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("cx", function (d) { return chart.x.scale(d.timestamp); })
            .attr("cy", function (d) { return chart.y.scale(d.point); })
            .style("fill", "#999999")
            .style("stroke", "#999999"); })
            .call(function (update) { return renderTimelineNetwork(update); }); }, function (exit) { return exit.remove(); });
        function renderTimelineNetwork(enter) {
            enter.selectAll(".circle-tag")
                ...
                .attr("r", 5)
                .style("fill", function (d) { return d.colour; })
                .style("stroke", function (d) { return d.colour; }); }, function (update) { return update.call(function (update) { return update.transition()
                .duration(750)
                .attr("cx", function (d) { return d.x; })
                .attr("cy", function (d) { return d.y; })
                .style("fill", function (d) { return d.colour; })
                .style("stroke", function (d) { return d.colour; }); }); }, function (exit) { return exit.remove(); });
        }
        chart.elements.content = chart.elements.contentContainer.selectAll(".circle");
        //Enable tooltip       
        _this.interactions.tooltip.enableTooltip(chart, onMouseover, onMouseout);
        function onMouseover(e, d) {
            ...
            _this.interactions.tooltip.appendTooltipContainer(chart);
            var tooltipValues = [new TooltipValues("Point", d.point)];
            var tags = Array.from(d3.rollup(d.tags, function (d) { return d.length; }, function (d) { return d.tag; }), function (_a) {
                var tag = _a[0], total = _a[1];
                return ({ tag: tag, total: total });
            });
            tags.forEach(function (c) {
                tooltipValues.push(new TooltipValues(c.tag, c.total));
            });
            var tooltipBox = _this.interactions.tooltip.appendTooltipText(chart, d.timestamp.toDateString(), tooltipValues);
            _this.interactions.tooltip.positionTooltipContainer(chart, xTooltip(d.timestamp, tooltipBox), yTooltip(d.point, tooltipBox));
            function xTooltip(x, tooltipBox) {
                var xTooltip = chart.x.scale(x);
                if (chart.width - chart.padding.yAxis < xTooltip + tooltipBox.node().getBBox().width) {
    ...
    AuthorControlCharts.prototype.renderNetwork = function (chart, data) {
        var _this = this;
        d3.select("#" + chart.id + " .card-subtitle")
            .html(data.nodes.filter(function (d) { return d.tag === "ref"; }).length == 1 ? "Filtering by <span class=\"badge badge-pill badge-info\">" + chart.x.scale.invert(data.nodes.find(function (d) { return d.tag === "ref"; }).fx).toDateString() + " <i class=\"fas fa-window-close\"></i></span>" :
            "");
        var links = chart.elements.contentContainer.selectAll(".network-link")
            .data(data.links)
            .join(function (enter) { return enter.append("line")
            .attr("class", "network-link")
            .classed("reflection-link", function (d) { return d.isReflection; })
            ...
            .attr("y2", function (d) { return d.target.y; }); }); }, function (update) { return update.call(function (update) { return update.classed("reflection-link", function (d) { return d.isReflection; })
            .transition()
            .duration(750)
            .attr("x1", function (d) { return d.source.x; })
            .attr("y1", function (d) { return d.source.y; })
            .attr("x2", function (d) { return d.target.x; })
            .attr("y2", function (d) { return d.target.y; }); }); }, function (exit) { return exit.remove(); });
        var nodes = chart.elements.contentContainer.selectAll(".network-node-group")
            .data(data.nodes)
            .join(function (enter) { return enter.append("g")
            .attr("class", "network-node-group")
            .attr("transform", "translate(" + chart.width / 2 + ", " + chart.height / 2 + ")")
            .call(function (enter) { return enter.append("rect")
            .attr("class", "network-node")
            .style("fill", function (d) { return d.colour; })
            .style("stroke", function (d) { return d.colour; }); })
            .call(function (enter) { return enter.append("text")
            .attr("id", function (d) { return "text-" + d.index; })
            .attr("class", "network-text"); })
            .call(function (enter) { return enter.select("rect")
            .attr("x", -5)
            .attr("y", -5)
            .attr("width", 10)
            .attr("height", 10); })
            .call(function (enter) { return enter.transition()
            .duration(750)
            .attr("transform", function (d) { return "translate(" + d.x + ", " + d.y + ")"; }); }); }, function (update) { return update.call(function (update) { return update.transition()
            .duration(750)
            .attr("transform", function (d) { return "translate(" + d.x + ", " + d.y + ")"; }); })
            .call(function (update) { return update.select("rect")
            .style("fill", function (d) { return d.colour; })
            .style("stroke", function (d) { return d.colour; }); })
            .call(function (update) { return update.select("text")
            .attr("id", function (d) { return "text-" + d.index; }); }); }, function (exit) { return exit.remove(); });
        ...
        function onMouseover(e, d) {
            ...
            var links = data.links.filter(function (d) { return d.source === d3.select(_this_1).datum(); }).map(function (d) { return d.target; });
            links = links.concat(data.links.filter(function (d) { return d.target === d3.select(_this_1).datum(); }).map(function (d) { return d.source; }));
            links.push(d3.select(this).datum());
            d3.selectAll(".network-node-group")
                .filter(function (d) { return links.includes(d); })
                .call(function (enter) { return enter.select("text")
                .text(function (d) { return d.phrase; })
                .style("opacity", 0)
                .transition()
                .duration(500)
                .style("opacity", "1"); })
                .call(function (enter) { return enter.select(".network-node")
                .transition()
                .duration(500)
                .attr("x", function (d) { return -(enter.select("#text-" + d.index).node().getBoundingClientRect().width + 10) / 2; })
                .attr("y", function (d) { return -(enter.select("#text-" + d.index).node().getBoundingClientRect().height + 5) / 2; })
                .attr("width", function (d) { return enter.select("#text-" + d.index).node().getBoundingClientRect().width + 10; })
                .attr("height", function (d) { return enter.select("#text-" + d.index).node().getBoundingClientRect().height + 5; }); });
        }
        function onMouseout() {
            var _this_1 = this;
            var links = data.links.filter(function (d) { return d.source === d3.select(_this_1).datum(); }).map(function (d) { return d.target; });
        ...
        _this.interactions.zoom.enableZoom(chart, zoomed);
        function zoomed(e) {
            var newChartRange = [0, chart.width - chart.padding.yAxis - chart.padding.right].map(function (d) { return e.transform.applyX(d); });
            chart.x.scale.rangeRound(newChartRange);
            chart.elements.contentContainer.selectAll(".network-link")
                .attr("x1", function (d) { return e.transform.applyX(d.source.x); })
                .attr("x2", function (d) { return e.transform.applyX(d.target.x); });
            chart.elements.contentContainer.selectAll(".network-node-group")
                .attr("transform", function (d, i, g) { return "translate(" + e.transform.applyX(d.x) + ", " + d.y + ")"; });
            chart.x.axis.ticks(newChartRange[1] / 75);
            chart.elements.xAxis.call(chart.x.axis);
            _this.help.removeHelp(chart);
        }
    ...
    AuthorControlCharts.prototype.renderReflections = function (data) {
        var _this = this;
        d3.select("#reflections .card-subtitle")
        ...
        d3.select("#reflections .reflections-tab")
            ...
            .call(function (div) { return div.append("p")
            .attr("class", "reflection-text")
            .html(function (d) { return _this.processReflectionsText(d); }); }); }, function (update) { return update.select("p")
            .html(function (d) { return _this.processReflectionsText(d); }); }, function (exit) { return exit.remove(); });
    };
    AuthorControlCharts.prototype.processReflectionsText = function (data) {
        var html = "<i>" + data.timestamp.toDateString() + " | Point: " + data.point + "</i><br>";
        for (var i = 0; i < data.text.length; i++) {
            var isOpenTag = data.tags.find(function (c) { return c.start_index === i; });
            var isCloseTag = data.tags.find(function (c) { return c.end_index === i; });
    ...
    return AuthorControlCharts;
}());
var AuthorControlTransitions = /** @class */ (function (_super) {
    __extends(AuthorControlTransitions, _super);
    function AuthorControlTransitions() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return AuthorControlTransitions;
}(Transitions));
var AuthorControlInteractions = /** @class */ (function (_super) {
    __extends(AuthorControlInteractions, _super);
    function AuthorControlInteractions() {
        var _this_1 = _super !== null && _super.apply(this, arguments) || this;
        _this_1.tooltip = new Tooltip();
        _this_1.zoom = new Zoom();
        return _this_1;
    }
    return AuthorControlInteractions;
}(AuthorControlTransitions));
var AuthorExperimentalCharts = /** @class */ (function (_super) {
    __extends(AuthorExperimentalCharts, _super);
    function AuthorExperimentalCharts() {
        var _this_1 = _super !== null && _super.apply(this, arguments) || this;
        _this_1.interactions = new AuthorExperimentalInteractions();
        _this_1.sorted = "date";
        return _this_1;
    }
    AuthorExperimentalCharts.prototype.preloadTags = function (entries, enable) {
        var tags = _super.prototype.preloadTags.call(this, entries, true);
    ...
    AuthorExperimentalCharts.prototype.handleTags = function () {
        var _this = this;
        d3.selectAll("#tags input[type=checkbox]").on("change", function (e) {
            var target = e.target;
            if (target.checked) {
                _this.allTags.find(function (d) { return d.tag == target.value; }).selected = true;
            }
            else {
                _this.allTags.find(function (d) { return d.tag == target.value; }).selected = false;
            }
            var entries = _this.getUpdatedEntriesData();
            var networkData = _this.getUpdatedNetworkData();
    ...
    ;
    AuthorExperimentalCharts.prototype.handleTagsColours = function () {
        var _this = this;
        d3.selectAll("#tags input[type=color]").on("change", function (e) {
            var target = e.target;
            var tag = target.id.replace("colour-", "");
            _this.allEntries = _this.allEntries.map(function (c) {
                var tags = c.tags.filter(function (d) { return d.tag === tag; });
                for (var i = 0; i < tags.length; i++) {
                    c.tags.find(function (d) { return d === tags[i]; }).colour = target.value;
                }
                return c;
            });
            var nodes = _this.allNetworkData.nodes.filter(function (d) { return d.tag === tag; });
    ...
    ;
    AuthorExperimentalCharts.prototype.handleReflectionsSort = function () {
        var _this = this;
        d3.select("#sort .btn-group-toggle").on("click", function (e) {
            var selectedOption = e.target.control.value;
            _this.allEntries = _this.allEntries.sort(function (a, b) {
    ...
    ;
    AuthorExperimentalCharts.prototype.handleFilterButton = function () {
        this.interactions.click.removeClick(this.timelineChart);
        var entries = this.getUpdatedEntriesData();
        var networkData = this.getUpdatedNetworkData();
    ...
    ;
    AuthorExperimentalCharts.prototype.getUpdatedEntriesData = function () {
        var _this = this;
        return _this.allEntries.map(function (c) {
            var tags = c.tags.filter(function (d) { return _this.allTags.filter(function (c) { return c.selected; }).map(function (r) { return r.tag; }).includes(d.tag); });
            return { "timestamp": c.timestamp, "pseudonym": c.pseudonym, "point": c.point, "text": c.text, "tags": tags, "matrix": c.matrix };
        });
    };
    AuthorExperimentalCharts.prototype.getUpdatedNetworkData = function (networkData) {
        var _this = this;
        var data = networkData === undefined ? _this.allNetworkData : networkData;
        var nodes = data.nodes.filter(function (d) {
            return _this.allTags.filter(function (c) { return c.selected; }).map(function (r) { return r.tag; }).includes(d.tag) || d.tag === "ref";
        });
        var links = data.links.filter(function (d) {
            return (_this.allTags.filter(function (c) { return c.selected; }).map(function (r) { return r.tag; }).includes(d.source.tag) &&
                _this.allTags.filter(function (c) { return c.selected; }).map(function (r) { return r.tag; }).includes(d.target.tag)) ||
                (d.source.tag === "ref" &&
                    _this.allTags.filter(function (c) { return c.selected; }).map(function (r) { return r.tag; }).includes(d.target.tag));
        });
        return { "nodes": nodes, "links": links };
    };
    AuthorExperimentalCharts.prototype.renderTimeline = function (chart, data) {
        chart = _super.prototype.renderTimeline.call(this, chart, data);
        var _this = this;
        _this.interactions.click.enableClick(chart, onClick);
        chart.elements.contentContainer.select(".zoom-rect").on("click", function () {
            _this.interactions.click.removeClick(chart);
        ...
        function onClick(e, d) {
            if (d3.select(this).attr("class").includes("clicked")) {
                _this.interactions.click.removeClick(chart);
                var networkData_1 = _this.getUpdatedNetworkData();
                _this.renderNetwork(_this.networkChart, networkData_1);
            ...
            var nodes = _this.allNetworkData.nodes.filter(function (c) {
                return filterNodes(d.tags, c) || c.phrase === d.timestamp.toDateString();
            });
            var links = _this.allNetworkData.links.filter(function (c) {
                return nodes.includes(c.source) || nodes.includes(c.target);
            });
            var networkData = _this.getUpdatedNetworkData({ "nodes": nodes, "links": links });
            _this.renderNetwork(_this.networkChart, networkData);
            _this.renderReflections([d]);
        }
        function filterNodes(tags, tag) {
            return tags.map(function (d) { return d.start_index; }).includes(tag.start_index) &&
                tags.map(function (d) { return d.end_index; }).includes(tag.end_index) &&
                tags.map(function (d) { return d.phrase; }).includes(tag.phrase);
        }
        return chart;
    };
    AuthorExperimentalCharts.prototype.renderNetwork = function (chart, data) {
        chart = _super.prototype.renderNetwork.call(this, chart, data);
        var _this = this;
        d3.select("#" + chart.id + " .badge").on("click", function () { return _this.handleFilterButton(); });
    ...
    AuthorExperimentalCharts.prototype.renderReflections = function (data) {
        _super.prototype.renderReflections.call(this, data);
        var _this = this;
        d3.select("#reflections .badge").on("click", function () { return _this.handleFilterButton(); });
    };
    return AuthorExperimentalCharts;
}(AuthorControlCharts));
var AuthorExperimentalInteractions = /** @class */ (function (_super) {
    __extends(AuthorExperimentalInteractions, _super);
    function AuthorExperimentalInteractions() {
        var _this_1 = _super !== null && _super.apply(this, arguments) || this;
        _this_1.click = new Click();
        _this_1.sort = new Sort();
        return _this_1;
    }
    return AuthorExperimentalInteractions;
}(AuthorControlInteractions));
    ...
    return __awaiter(this, void 0, void 0, function () {
        function drawCharts(allEntries) {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    data = allEntries.map(function (d) { return new AdminAnalyticsDataStats(d); });
                    //Render totals
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    loading = new Loading();
                    rawData = entriesRaw.map(function (d) { return new AdminAnalyticsDataRaw(d.group, d.value, d.createDate); });
                    entries = rawData.map(function (d) { return d.transformData(); });
                    colourScale = d3.scaleOrdinal(d3.schemeCategory10);
                    entries = entries.map(function (d) { return new AdminAnalyticsData(d.group, d.value, d.creteDate, colourScale(d.group), d.selected); });
        ...
        function drawCharts(allEntries) {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    data = entries.map(function (d) { return new AdminAnalyticsDataStats(d); });
                    //Render totals
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    entries = rawData.map(function (d) { return d.transformData(); });
                    colourScale = d3.scaleOrdinal(d3.schemeCategory10);
                    entries = entries.map(function (d) { return new AdminAnalyticsData(d.group, d.value, d.creteDate, colourScale(d.group), true); });
                    return [4 /*yield*/, drawCharts(entries)];
                case 1:
                    _a.sent();
...
function buildControlAuthorAnalyticsCharts(entriesRaw, analyticsRaw) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        function drawCharts(entries) {
            return __awaiter(this, void 0, void 0, function () {
                var authorControlCharts, networkChart, networkData, timelineChart;
                return __generator(this, function (_a) {
                    authorControlCharts = new AuthorControlCharts();
                    authorControlCharts.preloadTags(entries);
                    networkChart = new ChartNetwork("network", "chart-container-network", entries.map(function (d) { return d.timestamp; }));
                    networkData = authorControlCharts.processNetworkData(networkChart, entries);
                    ...
                    d3.select("#network .card-title button")
                        ...
                        var showDataHelp = authorControlCharts.help.helpPopover(networkChart.elements.contentContainer.select(".network-node-group"), networkChart.id + "-help-data", "<u><i>hover</i></u> me for information on demand");
                        if (showDataHelp) {
                            d3.select("#" + networkChart.id + "-help-data").style("top", parseInt(d3.select("#" + networkChart.id + "-help-data").style("top")) - 14 + "px");
                        }
                    });
                    timelineChart = new ChartTimeNetwork("timeline", entries.map(function (d) { return d.timestamp; }), new ChartPadding(40, 75, 10, 10));
                    entries.forEach(function (c) { return authorControlCharts.processTimelineSimulation(timelineChart, timelineChart.x.scale(c.timestamp), timelineChart.y.scale(c.point), c.tags); });
        ...
        var loading, colourScale, entries;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    loading = new Loading();
                    colourScale = d3.scaleOrdinal(d3.schemeCategory10);
                    entries = d3.sort(entriesRaw.map(function (d, i) { return { "timestamp": new Date(d.timestamp), "pseudonym": d.pseudonym, "point": d.point, "text": d.text, "tags": analyticsRaw[i].tags.map(function (d) { return processColour(d); }), "matrix": analyticsRaw[i].matrix }; }), function (d) { return d.timestamp; });
                    return [4 /*yield*/, drawCharts(entries)];
                case 1:
                    _a.sent();
...
exports.buildControlAuthorAnalyticsCharts = buildControlAuthorAnalyticsCharts;
function buildExperimentAuthorAnalyticsCharts(entriesRaw, analyticsRaw) {
    return __awaiter(this, void 0, void 0, function () {
        function processColour(tag) {
            if (tag.colour === undefined) {
                return { "start_index": tag.start_index, "tag": tag.tag, "phrase": tag.phrase, "colour": colourScale(tag.tag), "end_index": tag.end_index };
        ...
        function drawCharts(entries) {
            return __awaiter(this, void 0, void 0, function () {
                var authorExperimentalCharts;
                return __generator(this, function (_a) {
                    authorExperimentalCharts = new AuthorExperimentalCharts();
                    authorExperimentalCharts.preloadTags(entries, true);
                    authorExperimentalCharts.networkChart = new ChartNetwork("network", "chart-container-network", entries.map(function (d) { return d.timestamp; }));
                    ...
                    d3.select("#network .card-title button")
                        ...
                        var showDataHelp = authorExperimentalCharts.help.helpPopover(authorExperimentalCharts.networkChart.elements.contentContainer.select(".network-node-group"), authorExperimentalCharts.networkChart.id + "-help-data", "<u><i>hover</i></u> me for information on demand<br><u><i>drag</i></u> me to rearrange the network");
                        if (showDataHelp) {
                    ...
                    authorExperimentalCharts.timelineChart = new ChartTimeNetwork("timeline", entries.map(function (d) { return d.timestamp; }), new ChartPadding(40, 75, 10, 10));
                    entries.forEach(function (c) { return authorExperimentalCharts.processTimelineSimulation(authorExperimentalCharts.timelineChart, authorExperimentalCharts.timelineChart.x.scale(c.timestamp), authorExperimentalCharts.timelineChart.y.scale(c.point), c.tags); });
...
exports.buildExperimentAuthorAnalyticsCharts = buildExperimentAuthorAnalyticsCharts;
