...
//exports.buildExperimentAdminAnalyticsCharts1 = exports.buildExperimentAdminAnalyticsCharts = exports.buildControlAdminAnalyticsCharts = void 0;
//var d3 = require("d3");
...
var GroupCompareData = /** @class */ (function (_super) {
    __extends(GroupCompareData, _super);
    function GroupCompareData() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return GroupCompareData;
}(AnalyticsChartsData));
...
var Click = /** @class */ (function () {
    ...
    ;
    Click.prototype.appendText = function (chart, d, title, values) {
        if (values === void 0) { values = null; }
...
var AdminControlCharts = /** @class */ (function () {
    ...
    ;
    AdminControlCharts.prototype.handleGroups = function (boxPlot, allEntries, violin, usersViolin, timeline, timelineZoom) {
        var _this_1 = this;
...
var AdminExperimentalCharts = /** @class */ (function (_super) {
    ...
    AdminExperimentalCharts.prototype.handleGroups = function (boxPlot, allEntries, violin, usersViolin, timeline, timelineZoom, htmlContainers) {
        var _this = this;
        ...
        d3.selectAll("#groups input").on("change", function (e) {
            ...
            if (target.checked) {
                ...
                var experimentalCharts = updateCharts(boxPlot, data);
                if (boxPlot.click) {
                ...
                if (boxPlot.click) {
                    if (target.value == d3.select("#groups-statistics .card").attr("id")) {
                        ...
                        _this.interactions.click.appendGroupsText(boxPlot, data, data[data.map(function (d) { return d.group; }).indexOf(d3.select("#groups-statistics .card").attr("id"))]);
                        var violinData = _this.renderGroupCompare(data, d3.select("#groups-statistics .card").attr("id"));
                        experimentalCharts.violin.x = new ChartSeriesAxis("Group Code", violinData.map(function (r) { return r.group; }), [0, experimentalCharts.violin.width - experimentalCharts.violin.padding.yAxis - experimentalCharts.violin.padding.right]);
                        experimentalCharts.usersViolin.x = new ChartSeriesAxis("Group Code", violinData.map(function (r) { return r.group; }), [0, experimentalCharts.usersViolin.width - experimentalCharts.usersViolin.padding.yAxis - experimentalCharts.usersViolin.padding.right]);
    ...
    ;
    AdminExperimentalCharts.prototype.renderGroupExperimentalChart = function (chart, data, htmlContainers) {
        chart = _super.prototype.renderGroupChart.call(this, chart, data);
        var _this = this;
        var violinChart;
        var usersViolinChart;
        ...
        function onClick(e, d) {
            ...
            var groupsStatisticsCard = htmlContainers.appendCard(htmlContainers.groupStatistics, "Statitics (" + d.group + ")", d.group);
            _this.renderGroupStats(groupsStatisticsCard, d);
            //Draw compare
            htmlContainers.compare = htmlContainers.appendDiv("group-compare", "col-md-2 mt-3");
            var compareCard = htmlContainers.appendCard(htmlContainers.compare, "Compare " + d.group + " with:");
            compareCard.select(".card-body").attr("class", "card-body");
            var groupData = _this.renderGroupCompare(data, d.group);
            ...
            _this.renderViolin(violinChart, groupData);
            ...
            var usersData = groupData.map(function (d) {
                return d.getUsersData(d);
            });
            usersViolinChart = new ViolinChartSeries("group-violin-users-chart", groupData.map(function (d) { return d.group; }));
            ...
            _this.handleGroupCompare(groupData, violinChart, usersViolinChart);
            //Draw selected group timeline 
            htmlContainers.groupTimeline = htmlContainers.appendDiv("group-timeline", "col-md-12 mt-3");
            var timelineCard = htmlContainers.appendCard(htmlContainers.groupTimeline, "Reflections vs Time (" + d.group + ")");
        ...
        //Enable sort
        var sortButton = _this.interactions.sort.appendArrow(chart, false, true);
        ...
        return { groupChart: chart, violin: violinChart, usersViolin: usersViolinChart };
    };
    ;
    AdminExperimentalCharts.prototype.renderGroupCompare = function (data, id) {
        ...
        var compare = d3.select("#group-compare .card-body").selectAll("div")
            .data(compareData);
        compare.exit().remove();
        var compareEnter = compare.enter()
            .append("div")
            .attr("class", "form-check")
            .html(function (d) { return "<input class=\"form-check-input\" type=\"checkbox\" value=\"" + d.group + "\" id=\"compare-" + d.group + "\" " + (d.selected ? "checked" : "") + " />\n            <label class=\"form-check-label\" for=\"compare-" + d.group + "\">" + d.group + "</label>"; });
        compare.merge(compareEnter);
        return data.filter(function (d) { return compareData.includes(d) || d.group == id; });
    };
    ;
    AdminExperimentalCharts.prototype.handleGroupCompare = function (data, violin, usersViolin) {
        var _this = this;
        d3.selectAll("#group-compare input").on("change", function (e, d) {
            var target = e.target;
            if (target.checked) {
                data.find(function (d) { return d.group == target.value; }).selected = true;
            }
            else {
                data.find(function (d) { return d.group == target.value; }).selected = false;
            }
            var groupData = d3.filter(data, function (d) { return d.selected; });
            var usersData = groupData.map(function (d) { return d.getUsersData(d); });
            violin.x = new ChartSeriesAxis("Group Code", groupData.map(function (r) { return r.group; }), [0, violin.width - violin.padding.yAxis - violin.padding.right]);
            usersViolin.x = new ChartSeriesAxis("Group Code", groupData.map(function (r) { return r.group; }), [0, usersViolin.width - usersViolin.padding.yAxis - usersViolin.padding.right]);
...
function buildControlAdminAnalyticsCharts(entries) {
    ...
    function drawCharts(allEntries) {
        ...
        adminControlCharts.handleGroups(groupChart, allEntries.map(function (d) { return new AnalyticsChartsDataStats(d); }), violinChart, violinUsersChart, timelineChart, timelineZoomChart);
    }
}
...
function buildExperimentAdminAnalyticsCharts(entries) {
    ...
    function drawCharts(allEntries) {
        ...
        var groupChart = new ChartSeries("groups-chart", data.map(function (d) { return d.group; }));
        groupChart.elements.preRender(groupChart);
        var experimentalCharts = adminExperimentalCharts.renderGroupExperimentalChart(groupChart, data, htmlContainers);
        //Update charts depending on group
        adminExperimentalCharts.handleGroups(experimentalCharts.groupChart, allEntries.map(function (d) { return new AnalyticsChartsDataStats(d); }), experimentalCharts.violin, experimentalCharts.usersViolin, undefined, undefined, htmlContainers);
    }
}
//exports.buildExperimentAdminAnalyticsCharts = buildExperimentAdminAnalyticsCharts;
function buildExperimentAdminAnalyticsCharts1(entries) {
    //Handle sidebar button
    sidebarFunctions.sidebarBtn();
    //Draw charts
    drawCharts(entries);
...
//exports.buildExperimentAdminAnalyticsCharts1 = buildExperimentAdminAnalyticsCharts1;
