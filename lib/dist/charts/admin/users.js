...
export class Users {
    ...
    render() {
        d3.select(`#${this.id} .nav.nav-tabs`).selectAll("li")
            ...
            .html(d => d.group)
            .on("click", (e, d) => setTimeout(() => this.renderReflections(d3.select(`#reflections-${d.group}`), d, [30, 70]), 250)), update => update.attr("class", (d, i) => `nav-link ${i == 0 ? "active" : ""}`)
            .attr("href", d => `#reflections-${d.group}`)
            .attr("data-toggle", "tab")
        ...
        d3.select(`#${this.id} .tab-content`)
            ...
            .html(d => `<i>${d.timestamp.toDateString()} | Reflection point ${d.point}</i><br> ${d.text}`)))), update => update.attr("class", (d, i) => `tab-pane fade ${i == 0 ? "show active" : ""} users-tab-pane`)
            .attr("id", d => `reflections-${d.group}`)
            .call(update => update.selectAll(".statistics-text")
            .data(d => groupBy(d.value, "pseudonym"))
            .join(enter => enter, update => update.attr("id", d => d.key)
            ...
            .data(d => d3.sort(d.value, r => r.timestamp))
            .join(enter => enter, update => update.html(d => `<i>${d.timestamp.toDateString()} | Reflection point ${d.point}</i><br> ${d.text}`), exit => exit.remove())), exit => exit.remove())), exit => exit.remove());
