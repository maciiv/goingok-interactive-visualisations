...
export class Users {
    ...
    renderReflections(update) {
        update.selectAll("li")
            ...
            .html(d => `<i>${d.timestamp.toDateString()} | Reflection point ${d.point}</i><br> ${d.text}`), update => update.classed("reflection-selected", d => d.selected)
            .html(d => `<i>${d.timestamp.toDateString()} | Reflection point ${d.point}</i><br> ${d.text}`), exit => exit.remove());
    }
    renderReflections1(card, data, thresholds, timelineData) {
        let _this = this;
